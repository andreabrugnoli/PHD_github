...
Let $V$ be a finite dimensional normed linear space with norm $||\cdot||_V$. \\
\vspace{.1cm}
(If $V=\bbR^n$ then the Euclidean norm is denoted by $||x||_2 = \sqrt{x^\top x}$)

\begin{definition}[Locally integrable functions $L^p_{\text{loc}}$]
...
\begin{equation*}
	...
	\end{aligned} \qquad
	\begin{aligned}
		x(t) &\in \mathcal{X}, \qquad u(t) \in U, \\
		y(t) &\in Y,
	\end{aligned}
\end{equation*} 
with initial condition $x(0) = x_0$. In general $\mathcal{X}$ is a manifold and $U,\; Y$ vector spaces. \\
\vspace{.5cm}
For simplicity, in the following 
$$\mathcal{X} = \bbR^n,\qquad U=\bbR^m, \qquad Y=\bbR^p.$$

\begin{theorem}
	Suppose $f, h$ to be Lipschitz continuous in $x$ and $u$ jointly and $u(\cdot) \in L^2_{\text{loc}}(\bbR, U)$. \\
	Then system $\Sigma$ has a unique solution $\forall x_0 \in \mathcal{X}$ on a maximal interval $t \in [0, T(x_0))$ with $x(\cdot) \in L^2_{\text{loc}}(\bbR, \mathcal{X}), \ y(\cdot) \in L^2_{\text{loc}}(\bbR, Y)$.
\end{theorem}
...
The state space $\mathcal{X}$ is said to be \textbf{controllable} to $x_1$ if 
\begin{equation*}
...
\begin{frame}{Proof}
		...
		\begin{itemize}
			\item (If) Suppose $S_a$ is finite. Then $S_a \ge 0$ (sup of a set that contains 0).  Compare $S_a(x(t_0))$ and $S_a(x(t_1)) - \int_{t_0}^{t_1} s(u(t), y(t))\d t$ with $s(u, y)$ evaluated on a trajectory generated by $u: [t_0, t_1] \rightarrow U$ that drives $x(t_0)$ at $t_0$ to $x(t_1)$ at $t_1$. \\
		...
		\end{itemize}
		Then $S'= S - \inf_x S(x)$ satisfy the dissipation inequality so $S'(x) \ge S_a(x), \forall x$ and $\inf_x S'(x)=0$ (and hence $\inf_x S_a(x)=0$).
\end{frame}
...
\begin{proposition}
	If for a dissipative system it holds $S(x^∗)=0$, then it is dissipative from $x^*$. \\
	\vspace{.1cm}
	If a system is dissipative from $x^∗$, then $S_a(x^∗)=0$.	If additionally the system is reachable from $x^∗$, then the 	system is dissipative and its required supply satisfies $S_r(x^∗) = 0$.
\end{proposition}
\textbf{Proof} Assume $\Sigma$ is dissipative from $x^*$. By def. of $S_a$ if holds  $S_a(x^∗) = 0$. If is reachable from $x^*$, by the previous Pr. the system is dissipative, and $S_r(x^∗) = 0$.
...
\begin{frame}
	\begin{theorem}
		...
		\exists u(x) \; \text{  for which  } \;	s(u(x), h(x,u(x))) \le 0, \qquad x \in \mathcal{X},
		\end{equation}
		and $x^*$ is a globally asymptotically equilibrium for the closed-loop $\dot{x}̇ = f(x, u(x))$.\\ Then any storage function $S$ attains its minimum at $x^*$ and 
		\begin{equation*}
...
considering on $(T, \infty)$ a feedback $u(x)$ verifying \eqref{eq:s_neg} such that $x^*$ is a globally asymptotical equilibrium. \\
Since $s(u(x), h(x,u(x))) \le 0$ and $\lim_{t \rightarrow \infty} x(t) = x^∗$ then
\begin{equation*}
...
\begin{definition}[Stability]
	...
	\begin{enumerate}
	 ...
	 \begin{equation*}
	 	||x_0 - x^*||_2 \le \delta(\varepsilon) \implies ||x(t; x_0) - x^*||_2 < \varepsilon, \qquad \forall t \ge 0.
	 \end{equation*}
	 \item \textbf{asymptotically stable}, if it is stable and additionally there exists $\widehat{\delta}$ such that
	 \begin{equation*}
	 	||x_0 - x^*||_2 \le \widehat{\delta} \implies \lim_{t \rightarrow \infty} x(t; x_0) = x^*
	 \end{equation*}
...
\begin{frame}{First stability result}
	...
	\begin{proposition}
		Let $s(u, y)$ be a supply rate, and $S : \mathcal{X} \rightarrow \bbR_+$ be a $C^1$ storage
		function for $\Sigma$. Assume that $s$ satisfies
		\begin{equation*}
			s(0, y) \le 0, \qquad \forall y \in Y,
		\end{equation*}
		Assume that $x^* \in \mathcal{X}$ is an equilibrium for the unforced system $\dot{x} = f (x, 0)$. Then $x^*$ is a stable equilibrium of the unforced system  with Lyapunov function $V(x):=
...
\begin{frame}{Refinement via LaSalle}
	The condition $\dot{S} <0$ for asymptotic stability can be relaxed by using the LaSalle invariance principle.
	...
	\begin{theorem}[LaSalle's invariance principle]
		...
		f(x) \le 0, \, \forall \, x \in \mathcal{X}$. Suppose there exists a compact set $\mathcal{C}$ which is positively invariant for $\dot{x} = f(x)$. Then for any $x_0 \in \mathcal{C}$ the solution $x(t; x_0)$ converges
...
\begin{frame}
	\begin{proposition}
		Let  $S : \mathcal{X} \rightarrow \bbR_+$ be a $C^1$ storage
		function for $\Sigma$. Assume that $s$ satisfies
		\begin{equation*}
			s(0, y) \le 0, \qquad \forall y \in Y
		\end{equation*}
...
\begin{frame}{The open character of dissipativity theory}
	...
	$$s_i(u_i ,y_i), \qquad u_i \in U_i, \quad y_i \in Y_i,\quad  i = 1, \dots, k,$$
	...
	\begin{proposition}{Storage function for the interconnected systems}
...
\begin{frame}{The Lyapunov function of interconnected systems}
	
	For simplicity the spaces of external inputs and outputs are removed.	
	\begin{proposition}{Lyapunov function for the interconnected system}
...
\begin{frame}{Conclusions}
	...
	\begin{itemize}
		...
		\item  The storage function satisfies an \textbf{a priori bounds}. It is bounded from below by the available storage and from above by the required supply. These bounds possess a \textbf{variational characterization}.
