...
\begin{frame}{Mimetic dual-field discretization\footcite{brugnoli2022df}}
	\begin{itemize}
...
\centering
	\begin{tikzcd}
		...
		& H^{\curl}(M) \arrow[r, "\curl"] \arrow[u, "\flat"] \arrow[d, "\Pi_{s, h}^{-, 1}"] & H^{\div}(M) \arrow[r, "\div"] \arrow[u, "\beta"] \arrow[d, "\Pi_{s, h}^{-, 2}"]
		& L^2(M) \arrow[u, "\star^{-1}"] \arrow[d, "\Pi_{s, h}^{-, 1}"] \\
...
\begin{frame}{Illustration: the wave equation in 3D}
\begin{equation*}
	\begin{aligned}
		\begin{pmatrix}
			\partial_t p^3 \\
		...
		\end{pmatrix}, \qquad 
	\begin{aligned}
		\delta_{p} H^{0}|_{\Gamma_1} = u^0_1, \\
		-\delta_{\bm{u}} H^{2} \cdot \bm{n}|_{\Gamma_2}  = u^{2}_2,
	\end{aligned}\\
		y^2_2 &= -\delta_{\bm{u}} H^{2} \cdot \bm{n}|_{\Gamma_1}, \\
		y^{0}_1 &= \delta_{p} H^{0}|_{\Gamma_2}.
	\end{aligned}	
\end{equation*}
...
\caption*{Wave equation with mixed Dirichlet and Neumann boundary conditions}
\end{figure}
