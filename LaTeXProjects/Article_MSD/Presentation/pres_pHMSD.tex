...
\begin{itemize}
	...
	\item \onslide*<7->{Model reduction techniques not easily applicable.} 
\end{itemize}
\end{overlayarea}
\end{frame}
...
\begin{itemize}
	...
	\item \onslide*<3->{For control applications other approaches are too complex;}
	\item \onslide*<4->{Linear model reduction techniques are applicable.} 
\end{itemize}
Drawbacks:
\begin{itemize}
...
\subsection{Energy variables and momenta}
...
\onslide*<5-6>{&= \langle \bm{y}_\partial,  \bm{u}_\partial \rangle_{\mathscr{L}^2(\partial\Omega, \mathbb{R}^3)} + \langle \bm{\mathcal{B}}_d^* \bm{z}, \bm{u}_d \rangle_{\mathscr{X}} + \langle \bm{\mathcal{B}}_r^* \bm{z}, \bm{u}_\partial \rangle_{\mathscr{L}^2(\partial\Omega, \mathbb{R}^3)}, \quad \text{I.B.P. on } \bm{\mathcal{J}}, } \\
...
&= \int_{\Omega} \bm{u}_d \cdot \bm{y}_d \d\Gamma + \int_{\partial\Omega} \bm{u}_\partial \cdot (\bm{y}_\partial + \bm{y}_r)  \d{\Omega},
\end{aligned}
\end{equation*}
...
\end{itemize}
\centering
\Large Additional information\footfullcite{brugnoli2020msd} \url{https://arxiv.org/abs/2002.12816} \\
\end{frame}
