...
Consider an open connected set $\Omega \in \mathbb{R}^3$, representing a floating flexible body. The boundary $\partial \Omega = \Gamma_D \cup \Gamma_N$ is split into two subsets, one on which the surface traction is imposed ($\Gamma_N$ Neumann condition) and the other where the flexible displacement is known ($\Gamma_D$ Dirichlet condition).   The dynamics is computed at a generic point $P$, that is not necessarily the center of mass. The velocity of a generic point is expressed by considering a small flexible displacement superimposed to the rigid motion
\[
...
\begin{itemize}
	...
	&\quad - \crmat{\bm{\omega}_P} \crmat{\bm{\omega}_P} \bm{s}_u - \int_{\Omega} 2 \rho \crmat{\bm{\omega}_P} \dot{\bm{u}}_f \d{\Omega} +  \int_{\Omega} \bm\beta \d{\Omega} + \int_{\Gamma_N} \bm\tau \d{\Gamma},
	\end{split}
	\end{equation}
	...
	- \int_{\Omega} 2\rho \crmat{\bm{x}+\bm{u}_f} \crmat{\bm\omega_P} \dot{\bm{u}}_f \d{\Omega} + \int_{\Omega}\crmat{\bm{x}+\bm{u}_f} \bm\beta \d{\Omega} + \int_{\Gamma_N}\crmat{\bm{x}+\bm{u}_f} \bm\tau \d{\Gamma}, \\
	\end{split}
	\end{equation}
...
\left[m \bm{v}_P + \crmat{\bm{s}_u}^\top \bm\omega_P +2 \int_{\Omega} \rho \bm{v}_f \d{\Omega} \right]_\times \bm\omega_P +  \int_{\Omega} \bm\beta \d{\Omega} + \int_{\Gamma_N} \bm\tau \d{\Gamma}.
\end{split}
\end{equation}
\item Angular momentum balance:
\begin{equation}
...
\\
2 \int_{\Omega} \left[\rho \bm{v}_P + \rho \crmat{\bm{x}+\bm{u}_f}^\top \, \bm\omega_P \right]_\times \bm{v}_f \d{\Omega} + \int_{\Omega}\crmat{\bm{x}+\bm{u}_f} \bm\beta \d{\Omega} + \int_{\Gamma_N}\crmat{\bm{x}+\bm{u}_f} \bm{\tau} \d{\Gamma}.
\end{split}
\end{equation}
\item Flexibility PDE:
\begin{equation}
...
Consider the total energy (Hamiltonian), given by the sum of kinetic, deformation and potential energy:
\begin{equation}
\label{eq:H}
\begin{aligned}
...
For this reason the operator $\bm{\mathcal{J}}$ is skew-symmetric  $\bm{\mathcal{J}}_{}^*=-\bm{\mathcal{J}}$.


System \eqref{eq:ph_mfd_all} fits into the framework detailed in \cite{mehrmann2019structurepreserving} and extends it since a coupled system of ODEs and PDEs is considered. It can be rewritten compactly as follows
\begin{equation}
\label{eq:MFD_pHDAE}
\begin{aligned}
...
\end{aligned}
\end{equation}
where $\bm{u}_d = \bm\beta$. Using definitions  \eqref{eq:momenta} , it follows that the Hamiltonian  satisfies $\partial_{\bm{e}} H = \bm{\mathcal{E}}^* \bm{z}$. Adopting the same nomenclature as in \cite{mehrmann2019structurepreserving}, $\bm{e}$ contains the state and $\bm{z}$ contains the effort functions. The operators verify $\bm{\mathcal{E}} = \bm{\mathcal{E}}^*, \; \bm{\mathcal{J}} = -\bm{\mathcal{J}}^*$. The control operators are expressed as
\begin{align*}
	\bm{\mathcal{B}}_d &= 
	\begin{bmatrix}
	...
	\end{bmatrix}^\top, \\
	 \bm{\mathcal{B}}_r &= 
	 \begin{bmatrix}
	0 & 0 & 0 & \mathcal{I}^{\Gamma_N} & \bm{\mathcal{I}}_{x}^{\Gamma_N} & 0 & 0 \\
	0 & 0 & 0 & 0 & 0 & 0 & 0 \\ 
	 \end{bmatrix}^\top,
\end{align*}
where 
...
\mathcal{I}^{\Gamma_N} &:= \int_{\Gamma_N} (\cdot) \d{\Gamma}, \\
\end{aligned} \qquad
\begin{aligned} 
\bm{\mathcal{I}}_{x}^\Omega &:=\int_{\Omega} \crmat{\bm{x}+\bm{u}_f} (\cdot) \d{\Omega}, \\
\bm{\mathcal{I}}_{x}^{\Gamma_N} &:=\int_{\Gamma_N} \crmat{\bm{x}+\bm{u}_f} (\cdot) \d{\Gamma}. \\
\end{aligned}
\end{equation*}
The distributed control operator $\bm{\mathcal{B}}_d$  is compact. The boundary traction force acts on the rigid part through the compact operator $\bm{\mathcal{B}}_r$. The power Even if three dimensional elasticity has been taken as example up to this point, other models are easily considered. Beam and plate models \cite{BRUGNOLI2019940,BRUGNOLI2019961} are described by appropriate differential operators that replace the $\Div, \Grad$ appearing in \eqref{eq:ph_mfd_all} (see \secref{sec:ph_floatbeam}).
...
\begin{equation*}
\begin{aligned}
\bm{w}_1(\bm{x},t) = \bm{\phi}_1^\top(\bm{x}) \mathbf{w}_1(t), \\
\bm{e}_1(\bm{x},t) = \bm{\phi}_1^\top(\bm{x}) \mathbf{e}_1(t), 
\end{aligned} \qquad
\begin{aligned}
\bm{W}_2(\bm{x},t) = \bm{\phi}_2^\top(\bm{x}) \mathbf{w}_2(t), \\
\bm{E}_2(\bm{x},t) = \bm{\phi}_2^\top(\bm{x}) \mathbf{e}_2(t),
\end{aligned}
\end{equation*}
...
\begin{equation}
	\begin{aligned}
	\bm{E}(\bm{e}) \dot{\mathbf{e}} &= \bm{J}(\bm{e}) \bm{z}(\bm{e}) + \bm{B}_d(\bm{e}) \bm{u}_d + \bm{B}_\partial(\bm{e}) \bm{u}_\partial, \\
...
\begin{equation}
\langle y, \mathcal{A}x \rangle_{\mathscr{H}^2} = \langle \mathcal{A}^* y, x \rangle_{H^1}, \qquad x \in \mathscr{H}^1, y \in \mathscr{H}^2.
\end{equation}
\end{definition}
