...
Linear elastic structures have been largely investigated into the port-Hamiltonian (pH) framework as well as the heat equation (consult for instance \cite{macchelli2004timo} for the Timoshenko beam, \cite{aoues2017modeling} for the Euler-Bernoulli beam, \cite{brugnoli2019ammmin,brugnoli2019ammkir} for thick and thin plates). Recently, more complicated models arising from fluid dynamics have also been considered \cite{cardoso2020swe,rashad2020port1,rashad2020port2,altmann2017}. 


The Hamiltonian foundation of non-linear elasticity dates back to the late 80' \cite{simo1988}. However, the field of non-linear elasticity remains an unexplored topic in the pH framework 
...
\end{aligned}
\end{equation}
Notice that $w, \, e_w$ are discretized using the same test functions. Plugging \eqref{eq:Gal_appr} into \eqref{eq:weak_form}, the following finite dimensional system is obtained
...
\begin{table}[h]
	...
	\begin{tabular}{ccccc}
		...
		\hline 
		$E$ & $\rho$ & $L$  & $A$ & $I$ \\
		70 $[\textrm{GPa}]$ & $2700\; [\textrm{kg}/\textrm{m}^3]$ & 1 $[\textrm{m}]$ &  0.01 $[\textrm{m}^2]$ & $8.3 \; 10^{-6} \, [\textrm{m}^4]$\\ 
		\hline 
	\end{tabular} 
