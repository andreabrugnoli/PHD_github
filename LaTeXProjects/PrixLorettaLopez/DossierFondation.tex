...
Les personnes suivantes ont été contactés:

\begin{itemize}
\item Isabelle Bloy, Michele Colombo, Boin Jean-Philippe (AIRBUS);
\item Chistophe Robin (DAHER);
\item Christophe Valorge (Thales Alenia Space);
\item Paula Aguilera (CEA);
\item Corentin Lapeyre (CERFACS);
\item Volker Mehrmann (Président de l'association européenne des mathématiques et professeur à TU Berlin) qui a confirmé son soutien pour le projet;
\item Stefano Stramigioli (Vice Président of euRobotics et professeur à l'Université de Twente);
\end{itemize}
