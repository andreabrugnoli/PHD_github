...
\begin{remark}
	...
	\begin{bmatrix}
	0 & 0 & \bm{R} & 0 \\
	0 & 0 & 0 & \crmat{\bm{R}_{\mathrm{v}}} \\
	- \bm{R}^\top & 0 & 0 & \crmat{\bm{p}_t}\\
	0 & -\crmat{\bm{R}_{\mathrm{v}}}^\top & \crmat{\bm{p}_t} & \crmat{\bm{p}_r} \\
	\end{bmatrix}
	\begin{pmatrix}
...
\begin{equation}
\begin{aligned}
\inner[L^2(\Omega, \mathbb{R}^3)]{\bm{v}_1}{\rho \partial_t \bm{e}_1} &=- \left\langle {\Grad} (\bm{v}_1), \; \bm{E}_2 \right\rangle_{L^2(\Omega, \mathbb{R}^{3\times 3}_{\text{sym}})} + \inner[L^2(\Omega, \mathbb{R}^3)]{\bm{v}_1}{\bm{u}_\Omega}  + \left\langle \bm{v}_1, \; \bm{u}_\partial \right\rangle_{L^2(\partial \Omega, \mathbb{R}^3)}, \\
\inner[L^2(\Omega, \mathbb{R}^{3\times 3}_{\text{sym}})]{\bm{V}_2}{\bm{\mathcal{D}}^{-1}\partial_t \bm{e}_2} &=\left\langle \bm{V}_2, \; {\Grad} (\bm{e}_1) \right\rangle_{L^2(\Omega, \mathbb{R}^{3\times 3}_{\text{sym}})}.
\end{aligned}
\end{equation}

The output equation is discretized considering test function $\bm{v}_\partial$ defined over the boundary
\begin{equation}
\left\langle \bm{v}_\partial, \; \bm{y}_\partial \right\rangle_{L^2(\partial \Omega, \mathbb{R}^3)} = \left\langle \bm{w}_\partial, \; \bm{e}_1 \right\rangle_{L^2(\partial \Omega, \mathbb{R}^3)}.
\end{equation}
If a Galerkin method is applied then corresponding test and trial functions are discretized using the same basis

	\begin{equation*}
	\begin{aligned}
	\bm{v}_1(\bm{x}) &= \sum_{i=1}^{n_1} \bm{\phi}_1^i(\bm{x}) v_1^i, \\
	\bm{e}_1(\bm{x},t) &= \sum_{i=1}^{n_1} \bm{\phi}_1^i(\bm{x}) {e}_1^i(t), \\
	\end{aligned} \quad
	\begin{aligned}
	\bm{V}_2(\bm{x}) &= \sum_{i=1}^{n_2} \bm{\Phi}_2^i(\bm{x}) v_2^i, \\
	...
	\underbrace{\begin{bmatrix}
		...
		\mathbf{0} & \mathbf{M}_{\bm{\mathcal{D}}^{-1}} \\
	...
	\mathbf{u}_\partial, \\
	\mathbf{M}_{1} \widetilde{\mathbf{y}}_\Omega &= \begin{bmatrix}
	...
	\begin{aligned}
	M_\rho^{ij} &= \inner[L^2(\Omega, \bbR^3)]{\bm\phi_1^i}{\rho \bm\phi_1^j}, \\
	M_{\bm{\mathcal{D}}^{-1}}^{mn} &= \inner[L^2(\Omega, \bbR^{3\times 3}_{\text{sym}})]{\bm\Phi_2^m}{\bm{\mathcal{D}}^{-1}\bm\Phi_2^n}, \\
	M_1^{ij} &= \inner[L^2(\Omega, \bbR^3)]{\bm\phi_1^i}{\bm\phi_1^j}, \\
	\end{aligned}\quad
	\begin{aligned}
	M_\partial^{lk} &= \inner[L^2(\partial \Omega, \mathbb{R}^3)]{\bm\phi_\partial^l}{\bm\phi_\partial^k}, \\
	{D}_{\Grad}^{mi} &= \inner[L^2(\Omega, \bbR^{3\times 3}_{\text{sym}})]{\bm\Phi_2^m}{\Grad\bm\phi_1^i}, \\
	{B}_{1}^{ik} &= \inner[L^2(\partial \Omega, \mathbb{R}^3)]{\bm\phi_1^i}{\bm\phi_\partial^k}, \\
	\end{aligned}
	\end{equation}
...
\mathbf{y}_\Omega &:= \mathbf{M}_1 \widetilde{\mathbf{y}}_\Omega = \mathbf{B}_\Omega^\top \mathbf{e},  \\
...
\end{equation}
	...
	\begin{remark}[Connection with standard discretization]
		...
		\mathbf{M}_\rho \ddot{\mathbf{q}} + \mathbf{K} \mathbf{q} = \mathbf{M}_{1}\mathbf{u}_d + \mathbf{B}_{1}\mathbf{u}_\partial,
		\end{equation}
		where $\mathbf{M}_\rho, \; \mathbf{M}_{1}, \; \mathbf{B}_{1}$ are defined as in \eqref{eq:matrices}. Since the stiffness matrix $\mathbf{K}$ is symmetric and positive definite, it admits the following non unique factorization \cite{horn2012matrix} (see also \cite[Theorem 2]{cohen2005} for the equivalence of standard and mixed finite elements for elastodynamics problems)
		\begin{equation*} 
