...
\end{equation}
The inputs that assure the tracking of the outputs can be computed
\begin{equation}
\begin{aligned}
...
\begin{subequations}
	...
	\end{align}
\end{subequations}
This choice of the inputs assures that the outputs $y_{\partial}^1 = \partial_x e_w(0, t), \; y_{\partial}^2 = e_w(0, t)$ verify the desired trajectories
$$ y_{\partial}^1 =  \partial_t \partial_x w^r(0,t) = 0, \qquad y_{\partial}^2 = \partial_t w^r(0, t) = \omega \cos(\omega t).
$$
...
If a Lagrange multipliers is used for the Neumann boundary condition

\begin{equation}
\begin{aligned}
\inner[\Omega]{v_w}{\partial_t e_w} &= \inner[\Omega]{v_w}{-\partial_{xx} e_\kappa} , \\
\inner[\Omega]{v_\kappa}{\partial_t e_\kappa} &= \inner[L^2(\Omega)]{\partial_{xx} v_\kappa}{ e_w}  + \inner[\Gamma_N]{\begin{pmatrix}
...
For the decomposition, the beam is split into halves \ref{fig:dom_dec_EC}.
Applying the PFEM methodology as in \ref{sec:vdd} two finite dimensional systems are obtained. For $\Omega_N$ the system is analogous to \eqref{eq:pHfindim_Om2}, 

\begin{tcolorbox}[colframe=red,title=Subdomain $\Omega_N$, coltitle=white]%%
	\begin{equation}\label{eq:pHfindim_EB_OmN}
	\begin{aligned}
	\begin{bmatrix}
	\mathbf{M}_{w} & \mathbf{0} \\
	\mathbf{0} & \mathbf{M}_{\kappa} \\
	\end{bmatrix}
	\begin{pmatrix}
	\dot{\mathbf{e}}_w\\
	\dot{\mathbf{e}}_\kappa\\
	...
	{\mathbf{e}}_\kappa\\
	\end{pmatrix} + \begin{bmatrix}
	\mathbf{B}_{w, \Gamma_{\text{int}}}\\
	\mathbf{0}\\
	\end{bmatrix}\mathbf{u}_N^{\text{int}}, \\
	\mathbf{y}_N^{\text{int}} &=
	\begin{bmatrix}
	\mathbf{B}_{w, \Gamma_{\text{int}}}^\top & \mathbf{0}\\ 
	\end{bmatrix}
	\begin{pmatrix}
	{\mathbf{e}}_w\\
	{\mathbf{e}}_\kappa\\
	...
	\end{equation}
\end{tcolorbox}
while for $\Omega_D$ to \eqref{eq:pHfindim_Om1}.
...
\begin{tcolorbox}[colframe=blue,title=Subdomain $\Omega_D$,  coltitle=white]%%
	...
	\end{pmatrix} + \begin{bmatrix}
	\mathbf{0} & \mathbf{0} \\
	\mathbf{B}_{\kappa, \Gamma_D} & \mathbf{B}_{\kappa, \Gamma_{\text{int}}} \\
	\end{bmatrix} \begin{pmatrix}
	...
	\end{pmatrix}  &=
	\begin{bmatrix}
	\mathbf{0} & \mathbf{B}_{\kappa, \Gamma_D}^\top\\ 
	\mathbf{0} & \mathbf{B}_{\kappa, \Gamma_{\text{int}}}^\top\\ 
	\end{bmatrix}
	\begin{pmatrix}
	{\mathbf{e}}_w \\
	{\mathbf{e}}_\kappa \\
	...
	\end{equation}
\end{tcolorbox} 
In order to get a system with mixed causality, systems \eqref{eq:pHfindim_EB_OmD} and \eqref{eq:pHfindim_EB_OmD} have to be interconnected using a classical gyrator interconnection. The correct interconnection reads (cf. Fig. \ref{fig:dom_int_EB})
\begin{align*}
\mathbf{u}_N^{\text{int}} &= 
\begin{pmatrix}
...
Six elements are used for the discretization (both for the Lagrange multiplier and virtual domain decomposition method. The analytical solution for the reference dispacement and velocity together with their numerical discretization are plotted in Figs. \ref{fig:refw_EB} and \ref{fig:refv_EB}. The numerical predictions perfectly match the analytical solution.   In Fig. \ref{fig:w_motplan_EB} the numerical solution for the vertical displacement obtained using the virtual domain decomposition is shown. 
...
\begin{minipage}[t]{0.5\linewidth}
	\centering
	\includegraphics[width=0.98\columnwidth]{part_3/applications/mixed_bd_EB/ref_w.eps} \\
	\caption{Analytical reference displacement and numerical predictions.}
	\label{fig:refw_EB}
\end{minipage}
\begin{minipage}[t]{0.5\linewidth}
	\centering
	\includegraphics[width=0.98\columnwidth]{part_3/applications/mixed_bd_EB/ref_vel.eps} \\
...
\begin{tcbraster}[raster columns=2, raster equal height]
	\begin{tcolorbox}[width=0.5\textwidth, nobeforeafter, colframe=blue,title=Subdomain $\Omega_D$,  coltitle=white]%%
	\begin{equation}\label{eq:pHfindim_waves_OmD}
	\hspace*{-0.3cm}
	\begin{aligned}
	\mathbf{M}_{\Omega_D} \dot{\mathbf{e}}_{\Omega_D}= \mathbf{J}_{\Omega_D} \mathbf{e}_{\Omega_D} + \mathbf{B}^{\Omega_D}_{\Gamma_D} \mathbf{u}_D + \mathbf{B}^{\Omega_D}_{\Gamma_{\text{int}}} \mathbf{u}_D^{\text{int}}, \\
	...
	\hspace*{-0.3cm}
	\begin{aligned}
	\mathbf{M}_{\Omega_N}\dot{\mathbf{e}}_{\Omega_N} = \mathbf{J}_{\Omega_N} \mathbf{e}_{\Omega_N}  + \mathbf{B}^{\Omega_N}_{\Gamma_N} \mathbf{u}_N + \mathbf{B}^{\Omega_N}_{\Gamma_{\text{int}}} \mathbf{u}_N^{\text{int}}, \\
