...
\[
e_w(x,y,0) = x^2 \cos(2 \pi y), \qquad \bm{E}_\kappa(x,y,0) ={0}.
\]
...
The system is simulated using a St\"ormer-Verlet time integrator using a time step $\Delta t = 10^{-6}$ for a total simulation time of $t_{\text{end}} = 5 \; \mathrm{[s]}$. The Lagrange multiplier is eliminated using a projection method \cite{benner2015time} The control law is activated after 1 second. Snapshots of the simulation are reported in Fig. \ref{fig:SnapDamp_Kir}. The discrete Hamiltonian goes almost to zero in 4 seconds (Fig. \ref{fig:H_bs_Kirchhoff}).
...
\begin{figure*}[p]
	\centering
	\subfloat[$t=0.03 \; t_{\text{end}}$]{\includegraphics[height=0.2\textheight]{part_3/applications/bs_Kirchh/Snapshot_t30.eps}%
		\label{fig:Damp_1_Kir}}
	\subfloat[$t=0.06 \; t_{\text{end}}$]{\includegraphics[height=0.2\textheight]{part_3/applications/bs_Kirchh/Snapshot_t60.eps}%
		\label{fig:Damp_2_Kir}}
	\hfil
	\subfloat[$t=0.28 \; t_{\text{end}}$]{\includegraphics[height=0.2\textheight]{part_3/applications/bs_Kirchh/Snapshot_t280.eps}%
		\label{fig:Damp_3_Kir}}
	\subfloat[$t=0.32 \, t_{\text{end}}$]{\includegraphics[height=0.2\textheight]{part_3/applications/bs_Kirchh/Snapshot_t320.eps}%
		\label{fig:Damp_4_Kir}}
	\hfil
	\subfloat[$t=0.475 \; t_{\text{end}}$]{\includegraphics[height=0.2\textheight]{part_3/applications/bs_Kirchh/Snapshot_t475.eps}%
		\label{fig:Damp_5_Kir}}
	\subfloat[$t=0.57 \; t_{\text{end}}$]{\includegraphics[height=0.2\textheight]{part_3/applications/bs_Kirchh/Snapshot_t570.eps}%
		\label{fig:Damp_6_Kir}}
	\hfil
	\subfloat[$t=0.73 \; t_{\text{end}}$]{\includegraphics[height=0.2\textheight]{part_3/applications/bs_Kirchh/Snapshot_t730.eps}%
		\label{fig:Damp_7_Kir}}
	\subfloat[$t=0.965 \, t_{\text{end}}$]{\includegraphics[height=0.2\textheight]{part_3/applications/bs_Kirchh/Snapshot_t965.eps}%
...
\begin{equation*}
	k = 10^{-3}.
\end{equation*}
The control law is activated after $0.5$ seconds. The system is simulated using a Runge-Kutta method. Snapshots are collected in Fig. \ref{fig:SnapDamp_SWE}. The discretized Hamiltonian and Lyapunov functional trend (Fig. \ref{fig:HL_SWE}) clearly show that while the Lyapunov function monotonically decrease, the Hamiltonian does not.
...
\begin{figure*}[p]
	\centering
	\subfloat[$t=0/7 \; t_{\text{end}}$]{\includegraphics[height=0.2\textheight]{part_3/applications/bs_SWE/Snap_n1.eps}%
		\label{fig:Damp_1_SWE}}
	\subfloat[$t=1/7 \; t_{\text{end}}$]{\includegraphics[height=0.2\textheight]{part_3/applications/bs_SWE/Snap_n43.eps}%
		\label{fig:Damp_2_SWE}}
	\hfil
	\subfloat[$t=2/7 \; t_{\text{end}}$]{\includegraphics[height=0.2\textheight]{part_3/applications/bs_SWE/Snap_n86.eps}%
		\label{fig:Damp_3_SWE}}
	\subfloat[$t=3/7 \, t_{\text{end}}$]{\includegraphics[height=0.2\textheight]{part_3/applications/bs_SWE/Snap_n129.eps}%
		\label{fig:Damp_4_SWE}}
	\hfil
	\subfloat[$t=4/7 \; t_{\text{end}}$]{\includegraphics[height=0.2\textheight]{part_3/applications/bs_SWE/Snap_n171.eps}%
		\label{fig:Damp_5_SWE}}
	\subfloat[$t=5/7 \; t_{\text{end}}$]{\includegraphics[height=0.2\textheight]{part_3/applications/bs_SWE/Snap_n214.eps}%
		\label{fig:Damp_6_SWE}}
	\hfil
	\subfloat[$t=6/7 \; t_{\text{end}}$]{\includegraphics[height=0.2\textheight]{part_3/applications/bs_SWE/Snap_n257.eps}%
		\label{fig:Damp_7_SWE}}
	\subfloat[$t=7/7 \, t_{\text{end}}$]{\includegraphics[height=0.2\textheight]{part_3/applications/bs_SWE/Snap_n300.eps}%
...
We consider the model for the propagation of sound in air \ref{eq:pHsys_waves}
\begin{equation}
\begin{bmatrix}
\chi_s & p \\
\bm{0} & \mu_0 \\
...
\end{pmatrix},
\end{equation}
on $\Omega \subset \mathbb{R}^3$. $e_p \in \mathbb{R}$  and $\bm{e}_v \in \mathbb{R}^3$ denote the variations of pressure and velocity from a steady state, $\mu_0$ is the steady state mass density, and $\chi_s$ represents a constant adiabatic compressibility factor.  A cylindrical duct of length $L$ and radius $R$ is considered as physical domain, $\Omega = \{x \in [0, L], r \in [0, R], \theta = [0, 2 \pi)\}$. Denoting by $x, r, \theta$ the axial, radial and tangential coordinate,  the following boundary conditions are imposed
\begin{align}
e_p(x, R, \theta, t) &= - \mathcal{Z}(x, t) \, e_v^r(x, R, \theta), \label{eq:bc_imp} \\
\bm{e}_v \cdot \bm{n}(0, r, \theta, t) &= -e_v^x(0, r, \theta) = - f(r), \label{eq:bc_flowL}\\
\bm{e}_v \cdot \bm{n}(L, r, \theta, t) &= +e_v^x(L, r, \theta) = + f(r), \label{eq:bc_flowR}
\end{align}
...
\end{bmatrix}.
\end{equation}
The boundary conditions must now account for the symmetry condition at $r=0$, leading to the additional condition $\bm{e}_v\cdot \bm{n}(x, 0) = e_v^r(x, 0) =0$. The Hamiltonian is then computed as
\[
...
\begin{figure}[t]%
	\centering
	\includegraphics[width=0.45\columnwidth]{part_3/applications/mixed_bd_waves/boundary_part.eps} \\
	\caption{Boundary partition for the vibroacoustic problem.}
	\label{fig:bc_part_vibro}
\end{figure}

The boundary is split in two partition (see Fig. \ref{fig:bc_part_vibro}) to perform the discretization of this problem. In the next paragraphs we provide a concise description of the discretization procedure for the two methods.
...
\begin{figure}[t]%
	\centering
	\includegraphics[width=0.45\columnwidth]{part_3/applications/mixed_bd_waves/domain_split.eps} \\
	\caption{Virtual decomposition of the vibroacoustic domain.}
	\label{fig:dom_dec_vibro}
\end{figure}
Applying the PFEM methodology as in \ref{sec:vdd} two finite dimensional systems are obtained. For $\Omega_D$ the system is analogous to \eqref{eq:pHfindim_Om1} and reads
\begin{equation}
...
For $\Omega_N$ the system is analogous to \eqref{eq:pHfindim_Om2} and reads
\begin{equation}
...
\begin{equation}
\begin{aligned}{}
\mathbf{u}_N^{\text{int}} &= - \mathbf{y}_D^{\text{int}}, \\
...
\end{pmatrix},
\end{equation}
where $\mathbf{R}_{\Omega_D} = \mathbf{B}_{\Gamma_D}^{\Omega_D} \mathbf{M}_{\Gamma_D}^{-1} \mathbf{B}_{\Gamma_D}^{\Omega_D \top}$.
...
\begin{itemize}
	...
	\item $\bm{e}_v$ is interpolated using order~2 Raviart-Thomas polynomials;
...
Such a choice guarantees the conformity with respect to the differential operators. The matrices are obtained by using {\sc{FEniCS}}. The reference solution, obtained by using the DAE approach on a very fine mesh, is plotted in Fig. \ref{fig:Hdae15}, where the two contribution to the total energy
\[
...
Both methods converge monotonically to the reference solution, as illustrated in Figs. \ref{fig:p_err}, \ref{fig:v_err}. The faster convergence of one method on the other cannot be established.
