...
We consider the weak formulation \eqref{eq:weak_min_div}, reported in Section \secref{sec:linearPfem}. We present first a scheme that enforces the symmetry of the momenta tensor strongly and then a scheme in which the symmetry of the momenta tensor is imposed weakly.
...
\begin{remark}\label{rmk:BellArg}
	Thanks to a general approach for transforming finite elements \cite{kirby2018general}, $H^2$ conforming elements have been implemented in the {\sc{Firedrake}} library. 	Therefore, for the discretization of the $H^2$ space, the Argyris element \cite{argyris1968} is another valuable possibility.  Unfortunately, the strong imposition of boundary conditions is not possible in {\sc{Firedrake}} at the present time \cite[Sec. 3.2]{kirby2019}. Because of the simpler structure and ordering of its degrees of freedom, the Bell element has been privileged over the Argyris one for the convergence study. In Chap. \ref{ch:applications} the Argyris element will be employed imposing weakly the the boundary conditions.
\end{remark}
