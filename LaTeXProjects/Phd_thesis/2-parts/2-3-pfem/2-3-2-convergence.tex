...
\subsection{Mixed discretization for the weak form \eqref{eq:weak_EB_Hess}}
...
so that
\begin{equation}\label{eq:weak_EB_Hess_recall}
\begin{aligned}
...
Given an interval mesh $\mathcal{I}_h$ with elements $E$, the following family of finite elements is conforming for this problem
\begin{equation}
\label{eq:HerDG1}
\begin{aligned}
H_{h, \text{HerDG1}}^{2}(\Omega) &= \{ w_h \in H^{2}(\Omega) | \ \forall E \in \mathcal{I}_h,\ \bm{q}_h|_{Q} \in \mathrm{Her} \}, \\ 
L_{h, \text{HerDG1}}^2(\Omega) &= \{M_h \in L^2(\Omega) | \ \forall E \in \mathcal{I}_h, \ M_h|_{E} \in \mathrm{DG}_{1} \}, \\
\end{aligned}
\end{equation}
where Her denotes the cubic Hermite polynomials and DG is the discontinuous Galerkin finite element \cite[Chapter 3]{logg2012}.  Since for the discretization of the static problem the use of Hermite polynomial provides optimal convergence of order $2$, it seems logical to conjecture the following error estimates:

\begin{conjecture}[Convergence of the HerDG1 elements]\label{conj:HerDG1estimates}
	Assuming a smooth solution for problem~\eqref{eq:weak_EB_Hess_recall}, the following error estimates hold
	\begin{equation}
	\label{eq:errHerDG1}
	||e_w - e_w^h||_{L^{\infty} (H^2)} \lesssim h^{2}, \qquad
	||e_\kappa - e_\kappa^h||_{L^{\infty} (L^2)} \lesssim h^{2}.
	\end{equation}
\end{conjecture}


\subsection{Mixed discretization for the weak form \eqref{eq:weak_EB_divDiv}}
...
so that
\begin{equation}\label{eq:weak_EB_divDiv_recall}
\begin{aligned}
...
\end{aligned}
\end{equation}
Since the formulation is symmetrical to \eqref{eq:weak_EB_Hess_recall}, it seems logical to conjecture the following error estimates:

\begin{conjecture}[Convergence of the DG1Her elements]\label{conj:DG1Herestimates}
	Assuming a smooth solution for problem~\eqref{eq:weak_EB_divDiv_recall}, the following error estimates hold
	\begin{equation}
	\label{eq:errDG1Her}
	||e_w - e_w^h||_{L^{\infty} (L^2)} \lesssim h^{2}, \qquad
	||e_\kappa - e_\kappa^h||_{L^{\infty} (H^2)} \lesssim h^{2}.
	\end{equation}
\end{conjecture}
...
\begin{equation}
\begin{aligned}
H_{h, \text{CGCG}}^{2}(\Omega) &= \{ w_h \in H^{1}(\Omega) | \ \forall E \in \mathcal{I}_h,\ \bm{q}_h|_{Q} \in \mathrm{CG}_k \}, \\ 
L_{h, \text{CG}}^2(\Omega) &= \{M_h \in H^{1}(\Omega) | \ \forall E \in \mathcal{I}_h, \ M_h|_{E} \in \mathrm{CG}_k \}, \\
\end{aligned}
\end{equation}
...
\begin{conjecture}[Convergence of the CGCG elements]\label{conj:CGCGestimates}
	...
	||e_\kappa - e_\kappa^h||_{L^{\infty} (H^1)} \lesssim h^{k}.
	\end{equation}
\end{conjecture}
...
\section{Dual mixed discretization of plate problems}
...
\end{equation}
 ...
 Consider a regular triangulation $\mathcal{T}_h$ with elements $T$. The following family of finite elements is conforming to the weak formulation \eqref{eq:weak_min_grad}
\begin{equation}\label{eq:CGDG}
\begin{aligned}
H_{h}^{1}(\Omega) &= \{w_h \in H^1(\Omega) \vert \ \forall T \in \mathcal{T}_h, \ w_h|_{T} \in \mathrm{CG}_{k} \}, \\
H_h^{\Grad}(\Omega, \bbR^2) &= \{ \bm{\theta}_h \in H^{\Grad}(\Omega, \bbR^2) \vert \ \forall T \in \mathcal{T}_h,\ \bm{\theta}_h|_{T} \in (\mathrm{CG}_{k})^2 \}, \\
L_h^2(\Omega, \mathbb{S}) &= \{ \bm{M}_h \in L^2(\Omega, \mathbb{S}) \vert \ \forall T \in \mathcal{T}_h, \bm{M}_h|_{T} \in (\mathrm{DG}_{k-1})^{2 \times 2}_{\text{sym}} \}, \\
L_h^2(\Omega, \bbR^2) &= \{\bm{q}_h \in L^2(\Omega, \bbR^2)  \vert \ \forall T \in \mathcal{T}_h,\ \bm{q}_h|_{T} \in (\mathrm{DG}_{k-1})^{2} \}, \\
\end{aligned}
\end{equation}
To approximate spaces $H_h^{1}(\Omega), \; H_h^{\Grad}(\Omega, \bbR^2)$ Lagrange polynomials of order $k$ are selected. For spaces $L_h^2(\Omega, \mathbb{S}), \; L_h^2(\Omega, \bbR^2)$ Discontinous Galerkin polynomials of order $k-1$ are employed. We use the acronym CGDG (continuous Galerkin, discontinuous Galerkin) to denote this combination of elements
\[
...
In this section numerical test cases are used to verify the conjectured orders of convergence for the two problems. Upon discretization (cf. Section \secref{sec:linearPfem}), systems \eqref{eq:weak_min_div_strong}, \eqref{eq:weak_min_div_weak} and \eqref{eq:weak_min_grad} assumes the form 
\begin{equation*}
\underbrace{
	\begin{bmatrix}
...
\end{align*}
The dynamical solution is constructed as in Sec. \secref{sec:numtest_min}, meaning that a the static solution is multiplied by a sinusoidal function in time
\begin{equation*}
