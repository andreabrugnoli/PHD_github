...
\]
The notation
$$||u^\text{ex} - u_h || \lesssim  h^k$$
means $||u^\text{ex} - u_h|| \le C h^k$. The constant $C(u^\text{ex}, t_f)$ depends only on the exact solution $u^\text{ex}$ and on the final time $t_f$.
...
Consider a regular triangulation $\mathcal{T}_h$ with elements $T$. The following conforming family of finite elements is used to the weak formulation \eqref{eq:weak_min_grad_nobd}
...
\begin{figure}[htbp]%
	...
	\hspace{8pt}%
	\subfloat[][$L^\infty_{\Delta t} (H^2)$ error for $e_\kappa$]{%
...
Since the pH framework provides a powerful description of boundary control systems, it is important that numerical methods be capable of handling generic boundary conditions. For the Kirchhoff plate, a promising methodology is detailed in \cite{rafetseder2018siam}, but the dynamical case has not been considered yet. 