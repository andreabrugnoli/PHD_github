...
\end{aligned}
\end{equation}
where $\bm{x} \in \Omega$. The boundary variables are approximated as well
\begin{equation}\label{eq:approx_boundar}
\bm{v}_\partial \approx \sum_{i=1}^{n_\partial} \bm{\phi}_\partial^i(\bm{x}) v_\partial^i, \qquad
\bm{u}_\partial \approx \sum_{i=1}^{n_\partial} \bm{\phi}_\partial^i(\bm{x}) u_\partial^i(t), \qquad
\bm{y}_\partial \approx \sum_{i=1}^{n_\partial} \bm{\phi}_\partial^i(\bm{x}) y_\partial^i(t), \qquad \bm{x} \in \partial\Omega.
\end{equation}
...
M_2^{ij} &= \inner[X_2]{\bm\phi_2^i}{\bm\phi_2^j}, \\
\end{aligned}\quad
\begin{aligned}
{D}_{0}^{ji} &= \inner[X_2]{\bm\phi_2^j}{\bm{L}\bm\phi_1^i}, \\
{D}_{\mathcal{L}}^{ji} &= \inner[X_2]{\bm\phi_2^j}{\mathcal{L}\bm\phi_1^i}, 
\end{aligned}\quad  
\begin{aligned}
...
\begin{equation}
{D}_{-\mathcal{L}^*}^{ij} = \inner[X_1]{\bm\phi_1^i}{\mathcal{-L^*}\bm\phi_2^j}, \qquad B_2^{ik} = \inner[X_\partial]{\bm\phi_2^i}{\bm\phi_{\partial}^k} \qquad i \in \left\{1, n_1\right\}, \; j \in \left\{1, n_2\right\},\; k \in \left\{1, n_\partial \right\}.
\end{equation}
...
\end{aligned}
\end{equation}
Introducing a Galerkin approximation
\begin{equation}\label{eq:approx_ve_shwater}
\begin{aligned}
	{v}_h &\approx \sum_{i=1}^{n_1} {\phi}_h^i(\bm{x}) v_h^i, \\
	\bm{v}_v &\approx \sum_{i=1}^{n_2} \bm{\phi}_v^i(\bm{x}) v_v^i, \\
	{v}_\partial &\approx \sum_{i=1}^{n_\partial} {\phi}_\partial^i(\bm{s}) v_\partial^i
\end{aligned} \qquad 
\begin{aligned}
	{\alpha}_h &\approx \sum_{i=1}^{n_1} {\phi}_h^i(\bm{x}) \alpha_h^i(t), \\
	\bm{\alpha}_v &\approx \sum_{i=1}^{n_2} \bm{\phi}_v^i(\bm{x}) \alpha_v^i(t), \\
	{u}_\partial &\approx \sum_{i=1}^{n_\partial} {\phi}_\partial^i(\bm{s}) u_\partial^i(t)
\end{aligned} \qquad 
\begin{aligned}
	{e}_h &\approx \sum_{i=1}^{n_1} {\phi}_h^i(\bm{x}) e_h^i(t), \quad \bm{x} \in \Omega, \\
	\bm{e}_v &\approx \sum_{i=1}^{n_2} \bm{\phi}_v^i(\bm{x}) e_v^i(t), \quad \bm{x} \in \Omega,\\
	\bm{y}_\partial &\approx \sum_{i=1}^{n_\partial} {\phi}_\partial^i(\bm{s}) y_\partial^i(t), \quad \bm{s} \in \partial\Omega,
\end{aligned}
\end{equation}
...
M_v^{ij} &= \inner[L^2(\Omega, \mathbb{R}^2)]{\bm\phi_v^i}{\bm\phi_v^j}, \\
M_\partial^{lk} &= \inner[L^2(\partial\Omega)]{\phi_\partial^l}{\phi_\delta^k},
\end{aligned}\quad
\begin{aligned}
{D}_{\grad}^{ji} &= \inner[L^2(\Omega, \mathbb{R}^2)]{\bm\phi_v^j}{\grad \phi_h^i}, \\
{B}_{h}^{ik} &= \inner[L^2(\partial\Omega)]{\phi_h^i}{\phi_\partial^k}, \\
\end{aligned}
\end{equation}
where $i \in \left\{1, n_1\right\}, \; j \in \left\{1, n_2\right\}, \; l \in \left\{1, n_\partial \right\}, \; k \in \left\{1, n_\partial \right\}$. 
The discretized gradient of the Hamiltonian read
\begin{equation}
\begin{aligned}
\partial_{{\alpha}_{d, h}^i} H_d(\bm{\alpha}_{d, h},\; \bm{\alpha}_{d, v}) &= \inner[L^2(\Omega)]{\bm\phi_h^i}{\frac{1}{2 \rho} \norm{\sum_{i=1}^{n_2} \bm{\phi}_v^i \alpha_v^i}^2 + \rho g \sum_{i=1}^{n_1} {\phi}_h^i \alpha_h^i}, \qquad i \in \left\{1, n_1\right\}, \\
\partial_{{\alpha}_{d, v}^j} H_d(\bm{\alpha}_{d, h},\; \bm{\alpha}_{d, v}) &= \inner[L^2(\Omega, \mathbb{R}^2)]{\bm\phi_v^j}{\frac{1}{\rho} \left(\sum_{i=1}^{n_1} {\phi}_h^i \alpha_h^i\right) \left(\sum_{i=1}^{n_2} \bm{\phi}_v^i \alpha_v^i \right)}, \qquad j \in \left\{1, n_2\right\}.
\end{aligned}
\end{equation}
