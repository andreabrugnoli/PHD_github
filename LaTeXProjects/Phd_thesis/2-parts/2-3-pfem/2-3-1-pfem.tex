...
\begin{equation*}
X_1 = L^2(\Omega) \times L^2(\Omega, \mathbb{R}^2), \qquad X_2 = L^2(\Omega, \mathbb{R}^{2\times 2}_{\text{sym}}) \times L^2(\Omega, \mathbb{R}^2). 
\end{equation*}
...
\begin{remark}[Connectedness of $\Gamma_1, \Gamma_2$]
	Disconnected set can be handled as well. This requires the introduction of an heavy notation and complicates the illustration. For sake of simplicity, the connectedness hypothesis is then made.
\end{remark}
...
Since the boundary subsets $\Gamma_1, \; \Gamma_2$ are supposed to be connected set, a single interface is sufficient to decompose the system appropriately. In Fig. \ref{fig:dom_part} the splitting of the domain is accomplished by introducing the interface $\Gamma_{12}$. This separation line can be freely drawn. 
...
Suitable finite elements are required for both kind of discretization detailed in Sec. \ref{sec:pfem_gen}, but the two are not always available (see Remark \ref{rmk:HdivDiv}). A numerical convergence analysis of this technique looks rather involved.  Some particular cases of mixed conditions control cannot be handled. In particular, conditions on single components of vector cannot be imposed through this technique. For example, the simply supported condition in beams and plates set to zero tangential component of the traction at the boundary. The interface that separates the domain is an additional degree of freedom, as it can be freely drawn. However, as a general guideline, it has be such that the meshing of the subdomains generate sufficiently regular triangles. 
