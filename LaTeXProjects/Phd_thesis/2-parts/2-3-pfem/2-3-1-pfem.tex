...
\begin{figure}[tb]
	\centering
	\includegraphics[width=0.5\textwidth]{part_3/bound_part.eps}
	\caption{Partition of boundary into two connected sets.}
	\label{fig:bound_part}
\end{figure}

We will limit the illustration to the linear case. The open connected set $\Omega \subset \mathbb{R}^d, \; d=\{1,2,3\}$, with Lipschitz boundary $\partial\Omega$ represent the spatial domain. The boundary is split into two partition $\partial\Omega = \overline{\Gamma}_1 \cup \overline{\Gamma}_2, \; \Gamma_1 \cap \Gamma_2 = \{\emptyset\}$. For ease of presentation, $\Gamma_1, \; \Gamma_2$ are considered to be connected, cf. Fig. \ref{fig:bound_part}. Disconnected sets can be handled as well. For scalars ${a}_{\partial, 1}, {b}_{\partial, 1} \in L^2(\Gamma_1), \; {a}_{\partial, 2}, {b}_{\partial, 2} \in L^2(\Gamma_2)$ and  vectors  $\bm{a}_{\partial, 1}, \bm{b}_{\partial, 1} \in L^2(\Gamma_1, \mathbb{R}^m), \; \bm{a}_{\partial, 2}, \bm{b}_{\partial, 2} \in L^2(\Gamma_2, \mathbb{R}^m)$ defined on the boundary partition $\Gamma_1, \; \Gamma_2$ the inner product is defined as
\begin{equation}
\begin{aligned}
\inner[L^2(\Gamma_1)]{a_{\partial, 1}}{b_{\partial, 1}} = \int_{\Gamma_1} a_{\partial, 1} b_{\partial, 1} \d{\Gamma_1}, \qquad \inner[L^2(\Gamma_1, \mathbb{R}^m)]{\bm{a}_{\partial, 1}}{\bm{b}_{\partial, 1}} = \int_{\Gamma_1} \bm{a}_{\partial, 1} \cdot \bm{b}_{\partial, 1} \d{\Gamma_1}, \\
\inner[L^2(\Gamma_2)]{a_{\partial, 2}}{b_{\partial, 2}} = \int_{\Gamma_2} a_{\partial, 2} b_{\partial, 2} \d{\Gamma_2}, \qquad \inner[L^2(\Gamma_2, \mathbb{R}^m)]{\bm{a}_{\partial, 2}}{\bm{b}_{\partial, 2}} = \int_{\Gamma_2} \bm{a}_{\partial, 2} \cdot \bm{b}_{\partial, 2} \d{\Gamma_2}.  \\
\end{aligned}
\end{equation}   
...
&= \inner[X_{\Gamma_1}]{\bm{u}_{\partial, 1}}{\bm{y}_{\partial, 1}} + \inner[X_{\Gamma_2}]{\bm{y}_{\partial, 2}}{\bm{u}_{\partial, 2}},
\end{aligned} 
\end{equation}
where $X_{\partial\Omega}= L^2(\partial\Omega,\mathbb{R}^m), \; X_{\Gamma_1}= L^2(\Gamma_1,\mathbb{R}^m), \; X_{\Gamma_2}= L^2(\Gamma_2,\mathbb{R}^m)$. The continuous power balance is obtained using Eqs. \eqref{eq:powbal_lin} and \eqref{eq:split_bdterm}
\begin{equation}\label{eq:powbal_mixed}
	\dot{H} = \inner[X_{\Gamma_1}]{\bm{u}_{\partial, 1}}{\bm{y}_{\partial, 1}} + \inner[X_{\Gamma_2}]{\bm{y}_{\partial, 2}}{\bm{u}_{\partial, 2}}.
\end{equation}
...
\begin{aligned}
\inner[X_1]{\bm{v}_1}{\mathcal{M}_1 \partial_t \bm{e}_1} &=   -  \langle{\bm{v}_1}, \,{\bm{L}^\top \bm{e}_2}\rangle_{X_1}  -\inner[X_2]{\mathcal{L}\bm{v}_1}{\bm{e}_2} + \inner[X_{\partial\Omega}]{\mathcal{N}_{\partial, 1} \bm{v}_1}{\mathcal{N}_{\partial, 2}\bm{e}_2}, \\
\inner[X_2]{\bm{v}_2}{\mathcal{M}_2 \partial_t \bm{e}_2} &=   \inner[X_2]{\bm{v}_2}{\bm{L}\bm{e}_1} + \inner[X_2]{\bm{v}_2}{\mathcal{L}\bm{e}_1}, \\
\end{aligned}
\end{equation}
The term $\inner[X_{\partial\Omega}]{\mathcal{N}_{\partial, 1} \bm{v}_1}{\mathcal{N}_{\partial, 2}\bm{e}_2}$ can be split into the two boundary contributions, as in Eq. \eqref{eq:split_bdterm}. The variable  $\bm{y}_{\partial, 1}$ plays here the role of a Lagrange multiplier $\bm{y}_{\partial, 1}=\bm{\lambda}_{\partial, 1}$
\begin{equation}\label{eq:bdterm_mixed}
\begin{aligned}
\inner[X_{\partial\Omega}]{\mathcal{N}_{\partial, 1} \bm{v}_1}{\mathcal{N}_{\partial, 2}\bm{e}_2} &= \inner[X_{\Gamma_1}]{\mathcal{N}_{\partial, 1}^{\Gamma_1}\bm{v}_1}{\mathcal{N}_{\partial, 2}^{\Gamma_1}\bm{e}_2} + \inner[X_{\Gamma_2}]{\mathcal{N}_{\partial, 1}^{\Gamma_2}\bm{v}_1}{\mathcal{N}_{\partial, 2}^{\Gamma_2}\bm{e}_2}, \\
&= \inner[X_{\Gamma_1}]{\mathcal{N}_{\partial, 1}^{\Gamma_1}\bm{v}_1}{\bm{y}_{\partial, 1}} + \inner[X_{\Gamma_2}]{\mathcal{N}_{\partial, 1}^{\Gamma_2}\bm{v}_1}{\bm{u}_{\partial, 2}}, \\
&= \inner[X_{\Gamma_1}]{\mathcal{N}_{\partial, 1}^{\Gamma_1}\bm{v}_1}{\bm{\lambda}_{\partial, 1}} + \inner[X_{\Gamma_2}]{\mathcal{N}_{\partial, 1}^{\Gamma_2}\bm{v}_1}{\bm{u}_{\partial, 2}}, \\
\end{aligned}
\end{equation}
If test function $\bm{v}_{\partial, 1}, \; \bm{v}_{\partial, 2} \in X_{\Gamma_1} \times X_{\Gamma_2}$ are introduced, the input and outputs definitions
\begin{equation}
...
\begin{aligned}
\inner[X_{\Gamma_1}]{\bm{v}_{\partial, 1}}{\bm{u}_{\partial, 1}} &= \inner[X_{\Gamma_1}]{\bm{v}_{\partial, 1}}{\mathcal{N}_{\partial, 1}^{\Gamma_1}\bm{e}_{1}}, \\
\inner[X_{\Gamma_1}]{\bm{v}_{\partial, 1}}{\bm{y}_{\partial, 1}} &= \inner[X_{\Gamma_1}]{\bm{v}_{\partial, 1}}{\bm{\lambda}_{\partial, 1}}, \\
\inner[X_{\Gamma_1}]{\bm{v}_{\partial, 2}}{\bm{y}_{\partial, 2}} &= \inner[X_{\Gamma_1}]{\bm{v}_{\partial, 2}}{\mathcal{N}_{\partial, 1}^{\Gamma_2}\bm{e}_{1}}, \\
\end{aligned}
\end{equation}
...
\bm{\square}_{\partial, 2} &\approx \sum_{i=1}^{n_{\partial, 2}} \bm{\phi}_{\partial, 2}^i(\bm{s}_2) \square_\partial^i(t), \quad \bm{s}_2 \in \Gamma_2.
\end{aligned}
\end{equation}
where $\triangle$ stays for ${u}, {y}, {\lambda}$ and $\square$ for ${u}, {y}$. Replacing the approximation $\ref{eq:approx_vaeb_mixed}$ into Eqs. \ref{eq:pHlinsys_mixed_weak}, \ref{eq:bdterm_mixed}, \ref{eq:inpout_mixed}, the following system differential-algebraic system is obtained
\begin{equation}\label{eq:pHsys_infdim_mult}
\begin{aligned}
\mathrm{Diag}
\begin{bmatrix}
...
\begin{equation}
\begin{aligned}
{M}_{\partial, 1}^{lk} = \inner[X_{\Gamma_1}]{\bm{\phi}_{\partial, 1}^l}{\bm{\phi}_{\partial, 1}^k}, \quad (l,k) \in \{1,n_{\partial, 1}\}, \\
{M}_{\partial, 2}^{fg} = \inner[X_{\Gamma_2}]{\bm{\phi}_{\partial, 2}^f}{\bm{\phi}_{\partial, 2}^g}, \quad (f,g) \in \{1,n_{\partial, 2}\},\\
\end{aligned}  \qquad
\begin{aligned}
{B}_{1, \Gamma_1}^{ik} = \inner[X_{\Gamma_1}]{\bm{\phi}_{1}^i}{\bm{\phi}_{\partial, 1}^k}, \\
{B}_{1, \Gamma_2}^{ig} = \inner[X_{\Gamma_2}]{\bm{\phi}_{1}^i}{\bm{\phi}_{\partial, 2}^g}, \\
\end{aligned} \quad i \in \{1,n_1\},
\end{equation} 
the other matrices keep the same definition as in \eqref{eq:pHlinsys_findim_J1}. As in \cite{beattie2018linear}, the discretized Hamiltonian is then defined as
\begin{equation}
H_d = \frac{1}{2} \mathbf{e}_1^\top \mathbf{M}_{\mathcal{M}_1} \mathbf{e}_1 + \frac{1}{2} \mathbf{e}_2^\top \mathbf{M}_{\mathcal{M}_2} \mathbf{e}_2.
\end{equation}
The discrete power balance then equals
\begin{equation}
\begin{aligned}
...
&= \widehat{\mathbf{y}}_{\partial, 1}^\top \mathbf{u}_{\partial, 1} + \widehat{\mathbf{y}}_{\partial, 2}^\top \mathbf{u}_{\partial, 2}, \where \widehat{\mathbf{y}}_{\partial, 1} :=\mathbf{M}_{\partial, 1} \mathbf{y}_{\partial, 1}, \; \widehat{\mathbf{y}}_{\partial, 2} :=\mathbf{M}_{\partial, 2} \mathbf{y}_{\partial, 2}.
\end{aligned}
\end{equation}
...
\begin{equation}
{B}_{2, \Gamma_1}^{mk} = \inner[X_{\Gamma_1}]{\bm{\phi}_{2}^m}{\bm{\phi}_{\partial, 1}^k}, \quad
{B}_{2, \Gamma_2}^{mg} = \inner[X_{\Gamma_2}]{\bm{\phi}_{2}^m}{\bm{\phi}_{\partial, 2}^g}, 
 \quad m \in \{1,n_2\},\; k \in \{1,n_{\partial, 1}\},\; g \in \{1,n_{\partial, 2}\}.
\end{equation} 
