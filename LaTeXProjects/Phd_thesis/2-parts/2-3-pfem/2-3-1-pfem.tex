...
\label{eq:pHsys_2}
\begin{align}
\partial_t {\bm{\alpha}} &= \mathcal{J}\displaystyle \bm{e}, \qquad \;\;\, \bm{\alpha} \in X:=L^2(\Omega, \mathbb{F}), \quad \mathcal{J}: X \rightarrow X \vert \mathcal{J} = - \mathcal{J}^*, \label{eq:pHsys_dyn} \vspace{3pt}\\
\bm{e} :&= \displaystyle \delta_{\bm{\alpha}}H, \qquad \;\, \bm{e} \in H^\mathcal{J} :=\left\{\bm{e} \in X \vert \; \mathcal{J}\bm{e} \in X  \right\}, \label{eq:pHsys_const} \\
...
\end{align}
\end{subequations}
The operator $\mathcal{J}: X \rightarrow X$ is a differential, formally skew adjoint operator $\mathcal{J} = - \mathcal{J}^*$ over the space $X$. The Hilbert space $X$, whose inner product is denoted by $\inner[X]{\cdot}{\cdot}$, is a  of $L^2(\Omega, \mathbb{F})$ space over the field $\mathbb{F}$. The $\mathbb{F}$ field is an appropriate Cartesian product of either scalar, vectorial or tensorial quantities. Its precise definition depends on the example upon consideration. For scalars $(a,b) \in L^2(\Omega)$, vectors $(\bm{a}, \bm{b})\in L^2(\Omega, \mathbb{R}^d)$ and tensors  $(\bm{A}, \bm{B}) \in L^2(\Omega,\mathbb{R}^{d\times d})$ the $L^2$ inner product is given by 
\begin{equation}
...
\begin{assumption}\label{ass:linJ}
	Consider system \eqref{eq:pHsys_dyn}. It is assumed that the Hilbert space $X := L^2(\Omega, \mathbb{F})$ admits the splitting $X = X_1 \times X_2$ (meaning that the system is made up of two main blocks), where $X_1 := L^2(\Omega, \mathbb{A}), \; X_1 := L^2(\Omega, \mathbb{B})$. This means that $\mathbb{F} = \mathbb{A} \times \mathbb{B}$. \\
	
	The operator $\mathcal{J}$ is assumed to be skew-symmetric (or formally skew-adjoint) on $X$ and linear:
...
\begin{equation}
{D}_{-\mathcal{L}^*}^{im} = \inner[X_1]{\bm\phi_1^i}{\mathcal{-L^*}\bm\phi_2^m}, \quad B_2^{mk} = \inner[X_{\partial\Omega}]{\bm\phi_2^m}{\bm\phi_{\partial}^k}, \quad i \in \left\{1, n_1\right\}, \; m \in \left\{1, n_2\right\},\; k \in \left\{1, n_\partial \right\}.
\end{equation}
...
\begin{assumption}\label{ass:quadHam}
	...
	\begin{equation*}
	m_1 \bm{I}_1 \le\mathcal{Q}_1 \le M_1 \bm{I}_1, \qquad  m_2 \bm{I}_2 \le \mathcal{Q}_2 \le M_2 \bm{I}_2, \qquad m_1>0, \ m_2>0, \ M_1>0, \ M_2>0,
	\end{equation*} 
\end{assumption}
where $\bm{I}_1, \; \bm{I}_2$ are the identity operator in $X_1, \; X_2$ respectively. 
...
For this example, the Hilbert spaces under consideration are 
\begin{equation*}
X_1 = L^2(\Omega, \mathbb{R} \times \mathbb{R}^2), \qquad X_2 = L^2(\Omega, \mathbb{R}^{2\times 2}_{\text{sym}} \times \mathbb{R}^2). 
\end{equation*}
...
\begin{figure}[t]
	\begin{minipage}[b]{0.4\linewidth}
		\centering
		\includegraphics[width=0.95\textwidth]{part_3/dom_part.eps}
	...
	\begin{minipage}[b]{0.55\linewidth}
		\centering
			\includegraphics[width=0.95\textwidth]{part_3/dom_part_int.eps}
