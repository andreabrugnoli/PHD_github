...
\subsubsection{Kirchhoff plate}
The link beetween the energy and co-energy variables for the Kirchhoff model is the following
\begin{equation}
\alpha_w ={\rho h}e_w , \qquad \bm{A}_{\kappa} = \bm{\mathcal{C}}_b\bm{E}_{\kappa}
\end{equation}
where the bending compliance is given by 
\begin{equation}
\bm{\mathcal{C}}_b = \bm{\mathcal{D}}_b^{-1} = \frac{12}{E h^3}[(1 + \nu)(\cdot) - \nu \Tr(\cdot)\bm{I}_{2\times 2}].
\end{equation}
Given an open connected set $\Omega\subset \mathbb{R}^2$, the Kirchoff plate model \eqref{eq:phsysKir} in co-energy form controlled by forces and momenta is then expressed as 
\begin{subequations}\label{eq:pHlinsys_K1}
\begin{align}
\begin{bmatrix}
\rho h & 0 \\
0 & \bm{\mathcal{C}}_b
...
\begin{aligned}
e_w &\in H^2(\Omega),  \\
\bm{E}_\kappa &\in H^{\div\Div}(\Omega,\mathbb{S}), \label{eq:pHdyn_K1} \\
\end{aligned} 
 \vspace{3pt}\\
\bm{u}_\partial  &= 
	\begin{bmatrix}
...
\end{align}
\end{subequations}
where $\mathbb{S}=\mathbb{R}^{2\times 2}_{\text{sym}}$. We recall the expressions of the trace maps
\begin{equation}
\begin{aligned}
...
In this case $\mathcal{L}= \Hess, \; X_1=L^2(\Omega), \;  X_2=L^2(\Omega, \mathbb{S}),\; X_\partial = L^2(\partial\Omega, \mathbb{R}^2)$. The boundary operators $N_{\partial, 1}, \, N_{\partial, 2}$ are
\begin{equation}
N_{\partial, 1} = \begin{bmatrix}
\gamma_0 \\ \gamma_1
\end{bmatrix}, \qquad 
...
The energy rate is retrieve from Eq. \eqref{eq:enrateKir} $\dot{H} = \inner[L^2(\partial\Omega, \mathbb{R}^2)]{\bm{y}_\partial}{\bm{u}_\partial}$. Introducing the test functions $(v_w,\, \bm{V}_\kappa)$ and integrating by parts twice the first line of \eqref{eq:pHdyn_K1} one gets 
\begin{equation}
\begin{aligned}
\inner[L^2(\Omega)]{v_w}{\rho h \partial_t e_w} &= -\inner[L^2(\Omega, \mathbb{S})]{\Hess v_w}{\bm{E}_\kappa} + \inner[L^2(\partial\Omega)]{\gamma_0 v_w}{{u}_{q_n}} + \inner[L^2(\partial\Omega)]{\gamma_0 v_w}{{u}_{M_{nn}}}, \\
\inner[L^2(\Omega, \mathbb{S})]{\bm{V}_\kappa}{\bm{\mathcal{C}_b} \partial_t \bm{V}_\kappa} &= \inner[L^2(\Omega, \mathbb{S})]{\bm{V}_\kappa}{\Hess e_w}. \\
\end{aligned}
\end{equation}
Introducing a Galerkin discretization for test and efforts functions
\begin{equation}
...
\mathbf{0} & \mathbf{M}_{\mathcal{C}_b} \\
\end{bmatrix}
\begin{pmatrix}
\dot{\mathbf{e}}_{w} \\
\dot{\mathbf{e}}_{\kappa} \\
...
M_{\mathcal{C}_b}^{mn} = \inner[L^2(\Omega, \mathbb{S})]{\bm\Phi_\kappa^m}{\bm{\mathcal{C}}_b \bm\Phi_\kappa^n}, \quad D_{\Hess}^{mi} = \inner[L^2(\Omega)]{\bm\Phi_\kappa^m}{\Hess \phi_w^i}.
\end{equation}
The $\mathbf{B}_w$ is composed of two blocks $\mathbf{B}_w = [\mathbf{B}_w^{{q}_n} \; \mathbf{B}_w^{M_{nn}}]$
\begin{equation}
{B}_w^{q_n,il} = \inner[L^2(\partial\Omega)]{\gamma_0 \phi_w^i}{\phi_{\partial, 1}^l}, \qquad {B}_w^{M_{nn},il} = \inner[L^2(\partial\Omega)]{\gamma_1 \phi_w^i}{\phi_{\partial, 2}^l}, \qquad i \in \{1, n_w\}, \; l \in \{1, n_\partial\}.
\end{equation}
Equivalently, the second line of Eq. \eqref{eq:pHdyn_K1} can be integrated by parts twice to obtain a discretized system whose input are the linear velocity and the angular velocity at the boundary. However, while for the $H^2$ space conforming finite elements are available, for the $H^{\div\Div}$ no conforming finite elements have been proposed. This makes the discretization unfeasible. To handle a mixed boundary control, Lagrange multipliers may be used (cf. Section \secref{sec:lagrMul}).
