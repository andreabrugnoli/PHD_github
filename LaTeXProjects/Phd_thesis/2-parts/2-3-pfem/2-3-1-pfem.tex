...
\label{eq:pHsys_2}
\begin{align}
\partial_t {\bm{\alpha}} &= \mathcal{J}\displaystyle \bm{e}, \qquad \;\;\, \bm{\alpha} \in X:=L^2(\Omega, \mathbb{F}), \; \mathcal{J}: X \rightarrow X \vert \mathcal{J} = - \mathcal{J}^* \label{eq:pHsys_dyn} \vspace{3pt}\\
...
\begin{assumption}\label{ass:linJ}
	Consider system \eqref{eq:pHsys_dyn}. It is assumed that the Hilbert space $X := L^2(\Omega, \mathbb{F})$ admits the splitting $X = X_1 \times X_2$ (meaning that the system is made up of two main blocks), where $X_1 := L^2(\Omega, \mathbb{A}), \; X_1 := L^2(\Omega, \mathbb{B})$. This means than $\mathbb{F} = \mathbb{A} \times \mathbb{B}$. \\
