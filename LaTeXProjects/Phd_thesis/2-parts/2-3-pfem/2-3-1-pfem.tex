...
\begin{equation}
	...
	\end{pmatrix}.
\end{equation}
In this case $X_1 = X_2 = L^2(\Omega), \; X_\partial = \mathbb{R}^4$. The operators $\mathcal{L}, \, N_{\partial, 1}, \, N_{\partial, 2}$ read
\begin{equation}
...
Indeed, to lower the regularity requirement for the finite elements employed in the discretization, both lines can be integrated by parts. This will be discussed in Chap. \ref{ch:conv}.
...
The Hilbert spaces here considered are $X_1=L^2(\Omega), \;  X_2=L^2(\Omega, \mathbb{R}^{2\times 2}_{\text{sym}}),\; X_\partial = L^2(\partial\Omega, \mathbb{R}^2)$. The  operators $\mathcal{L}, \, N_{\partial, 1}, \, N_{\partial, 2}$ are
\begin{equation}
...
\end{aligned}
\end{equation}
The matrices $\mathbf{M}_{\rho h}, \; \mathbf{M}_{\mathcal{C}_b}, \; 
...
For this example, the Hilbert spaces under consideration are $X_1 = L^2(\Omega) \times L^2(\Omega, \mathbb{R}^2), \; X_2 = L^2(\Omega, \mathbb{R}^{2\times 2}_{\text{sym}}) \times L^2(\Omega, \mathbb{R}^2), \; X_\partial = L^2(\Omega, \mathbb{R}^3)$. The $\bm{L}, \; \mathcal{L}, \; \mathcal{N}_{\partial, 1}, \; \mathcal{N}_{\partial, 1}$ operators are 
\begin{equation}
\bm{L} = \begin{bmatrix}
\bm{0}  & \bm{0}  \\
...
\begin{equation}
\begin{aligned}
\inner[L^2(\Omega)]{v_w}{\rho h \partial_t e_w} &= -\inner[L^2(\Omega, \mathbb{R}^{2})]{\grad v_w}{\bm{e}_\theta} + \inner[L^2(\partial\Omega)]{\gamma_0 v_w}{{u}_{\partial, 1}}, \\
...
\begin{equation}
	\inner[L^2(\partial\Omega, \mathbb{R}^2)]{\bm\gamma_0 \bm{v}_\theta}{\bm\gamma_n \bm{E}_{\kappa}} = \inner[L^2(\partial\Omega)]{\bm\gamma_n \bm{v}_\theta}{\bm{u}_{\partial,2}} + \inner[L^2(\partial\Omega)]{\bm\gamma_s \bm{v}_\theta}{\bm{u}_{\partial,3}}
\end{equation}
Introducing a Galerkin discretization for test and efforts functions
...
\end{aligned} \qquad
\begin{aligned}
\quad M_{\mathcal{C}_b}^{pq} &= \inner[L^2(\Omega, \mathbb{R}^{2\times 2}_{\text{sym}})]{\bm\Phi_\kappa^p}{\bm{\mathcal{C}}_b \bm\Phi_\kappa^q}, \\
...
\begin{equation}
\begin{aligned}
\inner[L^2(\Omega)]{v_w}{\rho h \partial_t e_w} &= \inner[L^2(\Omega, \mathbb{R}^{2})]{v_w}{\div \bm{e}_\theta}, \\
...
\begin{equation}
\inner[L^2(\partial\Omega, \mathbb{R}^2)]{\bm\gamma_n \bm{V}_\kappa}{\bm\gamma_0 \bm{e}_{\theta}} = \inner[L^2(\partial\Omega)]{\gamma_{nn} \bm{V}_\kappa}{\bm{u}_{\partial,2}} + \inner[L^2(\partial\Omega)]{\gamma_{ns} \bm{V}_\kappa}{\bm{u}_{\partial,3}}
\end{equation}
...
\begin{equation}
{B}_{q_n}^{pl} = \inner[L^2(\partial\Omega)]{\gamma_n \bm\phi_\gamma^p}{\phi_{\partial, 1}^l}, \quad {B}_{M_{nn}}^{ml} = \inner[L^2(\partial\Omega)]{\gamma_{nn} \bm\Phi_\kappa^p}{\phi_{\partial, 2}^l},  \quad {B}_{\theta_s}^{ml} = \inner[L^2(\partial\Omega)]{\gamma_{ns} \bm\Phi_\kappa^m}{\phi_{\partial, 3}^l}.
\end{equation}

Constructing stable mixed finite element for this kind of discretization is a difficult task \cite{arnold2002mixed}. For this reason, many finite element discretization imposes the symmetry of the stress tensor weakly \cite{arnold2007mixed}. To actually implement the discretization, in Chap. \ref{ch:conv} the Mindlin plate problem is going to be reformulated so that the momenta tensor is only weakly symmetric. 
