\chapter{Elasticity in port-Hamiltonian form}\label{ch:elasPH}
...
A quantity of interest is the displacement $\bm{u}: \Omega \times [0, T_f] \rightarrow \mathbb{R}^d$ with respect to the reference configuration. It is defined as $\bm{u}(\bm{x}, t) = \bm{\Phi}(\bm{x}, t) - \bm{x}$ (cf. Fig. \ref{fig:non_lin}). The gradient of the displacement verifies $\nabla_x \bm{u} = \bm{F} - \bm{I}$. The strain tensor can now be written in terms of the displacement
\begin{equation*}
\begin{aligned}
...
\begin{figure}
	\centering
	\includegraphics[width=0.4\textwidth]{part_2/non_linear.eps}
	\caption{A 2D continuum in his reference configuration $\Omega$ and in the current configuration $\Omega_t$. Given a generic point $\bm{x}$ in the reference configuration, the map $\bm{\Phi}(\bm{x}, t)$ provides its current position $\bm{y}$ at a given time instant $t$.}
	\label{fig:non_lin}
\end{figure}
...
\begin{proposition}\label{prop:varder_tens}
	...
	\begin{proof}
		...
		\begin{equation*}H_{\mathrm{def}}(\bar{\bm{A}}_{\varepsilon} + \eta \bm{\Delta}\bm{A}_{\varepsilon}) = H_{\mathrm{def}}(\bar{\bm{A}}_{\varepsilon}) + \eta \inner[L^2(\Omega, \mathbb{S})]{\diffd{H_{\mathrm{def}}}{\bm{A}_{\varepsilon}}}{\bm{\Delta}\bm{A}_{\varepsilon}} + O(\eta^2), \end{equation*}
		
		Then, by identification 
		\begin{equation*}
		\diffd{H_{\mathrm{def}}}{\bm{A}_{\varepsilon}} = \bm{\mathcal{D}} \bar{\bm{A}}_{\varepsilon} = \bm{\Sigma}.
		\end{equation*}
		Since the Hamiltonian is separable $\delta_{\bm{A}_{\varepsilon}}{H_{\mathrm{def}}} =\delta_{\bm{A}_{\varepsilon}} H$, leading to the final result.
	\end{proof}
\end{proposition}
...
&= \int_{\partial \Omega} \bm{e}_v \cdot (\bm{E}_\varepsilon\bm{n}) \d{S}.
\end{aligned}
\end{equation}
...
\begin{remark}\label{rmk:unboundednessBC}
	The boundary operators $\mathcal{B}_\partial, \mathcal{C}_\partial$ in Eq.\eqref{eq:pHsysElas} are unbounded if the co-energy belong to $H^{\Grad}, \; H^{\Div}$.
\end{remark}
...
\begin{remark}[Trace operators and duality pairing for the elastodynamics problem]\label{rmk:duality_elas}
	...
	\inner[L^2(\Omega, \bbS)]{\Grad \bm{e}_v}{\bm{E}_\varepsilon} + \inner[L^2(\Omega, \mathbb{V})]{\bm{e}_v}{\Div \bm{E}_\varepsilon} = \inner[H^{\frac{1}{2}} \times H^{-\frac{1}{2}}(\partial\Omega, \mathbb{V})]{\bm\gamma_{0} \bm{e}_v}{\bm\gamma_{n}\bm{E}_\varepsilon},
	\end{equation}
...
\begin{theorem}[Stokes-Dirac structure for elastodynamics] \label{thm:stdirElas}
...
V_\partial &:= H^{\frac{1}{2}}(\partial\Omega, \mathbb{V}), \\
V_\partial' &:= H^{-\frac{1}{2}}(\partial\Omega, \mathbb{V}), \\
\end{align*}
...
\end{pmatrix}
\vert \;
 \bm{e} \in H, \quad \bm{f} = \mathcal{J} \bm{e} \in F, \quad \bm{f}_\partial  = \bm{\gamma}_0 \bm{e}_v \in V_\partial, \quad \bm{e}_\partial = -\bm{\gamma}_n \bm{E}_\varepsilon \in V_\partial'  \right\},
\end{equation}
where $\bm{e} = (\bm{e}_v,\, \bm{E}_\varepsilon)$ and $\mathcal{J}$ are defined in \eqref{eq:pHsysElas}, is a Stokes–Dirac structure with respect to the pairing
\begin{equation}\label{eq:bilinearElas}
\begin{aligned}
\bilprod{(\bm{f}^1, \bm{f}_{\partial}^1, \bm{e}^1, \bm{e}_{\partial}^1)}{(\bm{f}^2, \bm{f}_{\partial}^2, \bm{e}^2, \bm{e}_{\partial}^2)}  := &\inner[F]{\bm{f}^1}{\bm{e}^2} + \inner[F]{\bm{f}^2}{\bm{e}^1} \\
 + &\inner[V_\partial \times V_\partial']{\bm{f}_{\partial}^1}{\bm{e}_{\partial}^2} + \inner[V_\partial \times V_\partial']{\bm{f}_{\partial}^2}{\bm{e}_{\partial}^1},
\end{aligned}
\end{equation}
where
\begin{equation}
\inner[V_\partial \times V_\partial']{\bm{f}_{\partial}}{\bm{e}_{\partial}} = -  \inner[H^{\frac{1}{2}} \times H^{-\frac{1}{2}}(\partial\Omega, \mathbb{V})]{\bm\gamma_{0} \bm{e}_v}{\bm\gamma_{n}\bm{E}_\varepsilon}.
\end{equation}
\end{theorem}
...
\begin{proof}
	...
	\begin{align*}
	\bilprod{(\bm{f}, \bm{f}_{\partial}, \bm{e}, \bm{e}_{\partial})}{(\bm{f}, \bm{f}_{\partial}, \bm{e}, \bm{e}_{\partial})} =& 2 \inner[F]{\bm{f}}{\bm{e}} + 2 \inner[V_\partial \times V_\partial']{\bm{f}_{\partial}}{\bm{e}_{\partial}}, \\
	=& 2 \inner[F]{\mathcal{J}\bm{e}}{\bm{e}} + 2 \inner[V_\partial \times V_\partial']{\bm{f}_{\partial}}{\bm{e}_{\partial}}, \\
	...
	= 0, \quad \text{from \eqref{eq:duality_elas}}.
	\end{align*}
	This implies ${D}_{\mathcal{J}} \subset {D}_{\mathcal{J}}^\perp$.
	
	\textit{Step 2}. Take $(\bm{\phi}, \, \bm{\phi}_\partial, \, \bm{\epsilon}, \, \bm{\epsilon}_\partial) \in {D}_{\mathcal{J}}^\perp$ and $\bm{e}_0 = (\bm{e}_{v, 0}, \; \bm{E}_{\varepsilon, 0}) \in H$ with compact support on $\Omega$. This implies $\bm{\gamma_{0}} \bm{e}_{v, 0} = \bm{0}$ and $\bm{\gamma}_n \bm{E}_{\varepsilon, 0} = \bm{0}$. Taking $(\mathcal{J}\bm{e}_0, \bm{0}, \bm{e}_0, \bm{0}) \in {D}_{\mathcal{J}}$ then 
	\begin{equation*}
	\bilprod{(\bm{\phi}, \bm{\phi}_\partial,  \bm{\epsilon}, \bm{\epsilon}_\partial)}{(\mathcal{J}\bm{e}_0, \bm{0}, \bm{e}_0, \bm{0})} = \inner[F]{\mathcal{J}\bm{e}_0}{\bm{\epsilon}} + \inner[F]{\bm{\phi}}{\bm{e}_0} = 0, \quad \forall \bm{e}_0 \in H.
	\end{equation*}
	...
	\begin{align*}
	0 &=\inner[F]{\mathcal{J}\bm{\epsilon}}{\bm{e}} + \inner[F]{\mathcal{J}\bm{e}}{\bm{\epsilon}}+ \inner[V_\partial \times V_\partial']{\bm{\phi}_{\partial}}{\bm{e}_{\partial}} +  \inner[V_\partial \times V_\partial']{\bm{f}_{\partial}}{\bm{\epsilon}_{\partial}}, \\
	&=\int_{\partial\Omega} \left\{\bm{e}_v \cdot (\bm{\mathcal{E}}_\varepsilon \, \bm{n}) + \bm{\epsilon}_v \cdot (\bm{{E}}_\varepsilon \, \bm{n})\right\}  \d{S} + \inner[V_\partial \times V_\partial']{\bm{\phi}_{\partial}}{-\bm{\gamma}_{n}\bm{E}_\varepsilon} +  \inner[V_\partial \times V_\partial']{\bm{\gamma}_0\bm{e}_v}{\bm{\epsilon}_{\partial}}.
	\end{align*}
	Given the fact that $\bm{e} = (\bm{e}_v, \; \bm{E}_\varepsilon)$ is arbitrary then
	\begin{equation*}
	\bm{\phi}_{\partial} = \bm\gamma_{0} \bm{\epsilon}_v, \qquad 
	\bm{\epsilon}_{\partial} = -\bm\gamma_n	\bm{\mathcal{E}}_\varepsilon,
	\end{equation*}
	meaning that ${D}_{\mathcal{J}}^\perp \subset {D}_{\mathcal{J}}$. 
\end{proof}

\begin{remark}[General mixed causality and existence and uniqueness of solutions]\label{rmk:mix_ExUn}
The proof consider uniform boundary condition based on the integration by parts formula given in Remark \ref{rmk:duality_elas}. This is sufficient for characterizing the Stokes-Dirac structure. Indeed, for the existence and uniqueness of solution of system \eqref{eq:pHsysElas}, the control inputs and outputs have to satisfy additional compatibility conditions \cite{grisvard2011}. In particular, when both Neumann and Dirichlet conditions are imposed and their boundary partitions touch (just like in Fig. \ref{fig:bc_elas2D}), then the appropriate space for the Dirichlet control is $H^{1/2}_{00}$ (see e.g. \cite{ramaswamy2019}). The conjugated output will then belong to the topological dual of this space, that does not possess a characterization. These issues are out of the scope of this work.
\end{remark}
