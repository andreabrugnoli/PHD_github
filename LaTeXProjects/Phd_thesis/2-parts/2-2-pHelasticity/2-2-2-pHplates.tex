\chapter{Port-Hamiltonian plate theory}\label{ch:platePH}
...
\end{align}
where $\bm{u}^0 = (u_x, u_y)^\top, \; \bm{\theta} = (\theta_x, \theta_y)^\top$. For now, it is assumed that the material is isotropic, linear elastic (in \secref{sec:lamAnis} this hypothesis is removed). Recall the Hooke's law for 3D continua (see Eq. \eqref{eq:stiff3D})
\begin{equation*}
...
\begin{remark}[Trace operators and duality pairings for the Mindlin plate]\label{rmk:duality_min}
	...
	\begin{aligned}
	\inner{\grad {e}_w}{\bm{e}_\gamma} + \inner{e_w}{\div \bm{e}_\gamma} +  \inner{\Grad \bm{e}_\theta}{\bm{E}_\kappa} + \inner{\bm{e}_\theta}{\Div \bm{E}_\kappa} = &+\inner[H^{\frac{1}{2}} \times H^{-\frac{1}{2}}(\partial\Omega)]{\gamma_{0} {e}_w}{\gamma_{n}\bm{e}_\gamma} \\
	&+\inner[H^{\frac{1}{2}} \times H^{-\frac{1}{2}}(\partial\Omega, \mathbb{V})]{\bm\gamma_{0} \bm{e}_\theta}{\bm\gamma_{n}\bm{E}_\kappa}, \\
	\end{aligned}
	\end{equation}
...
\begin{theorem}[Stokes-Dirac structure for the Mindlin plate]\label{thm:stdirMin}
...
V_\partial &:= H^{\frac{1}{2}}(\partial\Omega) \times H^{\frac{1}{2}}(\partial\Omega, \mathbb{V}), \\
V_\partial' &:= H^{-\frac{1}{2}}(\partial\Omega) \times H^{-\frac{1}{2}}(\partial\Omega, \mathbb{V}) .
\end{align*}
...
\end{pmatrix}
\vert \;
\bm{e} \in H, \quad \bm{f} = \mathcal{J} \bm{e} \in F,  \quad \bm{f}_\partial &= \begin{bmatrix}
\gamma_0 & 0 \\
\bm{0} & \bm{\gamma}_0 \\ 
\end{bmatrix}
\begin{pmatrix}
e_w \\ \bm{e}_\theta \\ 
\end{pmatrix} \in V_\partial, \\ 
\bm{e}_\partial &= -\begin{bmatrix}
\bm{\gamma}_n & 0 \\
\bm{0} & \gamma_n \\ 
\end{bmatrix}
\begin{pmatrix}
\bm{E}_\kappa \\ \bm{e}_\gamma \\  
\end{pmatrix} \in V_\partial' \Biggr\},
\end{aligned}
\end{equation}
where $\bm{e} = ({e}_w, \,\bm{e}_\theta, \, \bm{E}_\kappa, \, \bm{e}_\gamma)$ and $\mathcal{J}$ is defined in \eqref{eq:pHsysMin}, is a Stokes–Dirac structure with respect to the pairing
...
where
\begin{equation*}
\inner[V_\partial \times V_\partial']{\bm{f}_{\partial}}{\bm{e}_{\partial}} = -\inner[H^{\frac{1}{2}} \times H^{-\frac{1}{2}}(\partial\Omega)]{\gamma_{0} {e}_w}{\gamma_{n}\bm{e}_\gamma} -\inner[H^{\frac{1}{2}} \times H^{-\frac{1}{2}}(\partial\Omega, \mathbb{V})]{\bm\gamma_{0} \bm{e}_\theta}{\bm\gamma_{n}\bm{E}_\kappa}.
\end{equation*}
\end{theorem}

\begin{proof}
	The proof is analogous to the proof of to Theorem \ref{thm:stdirElas}. Here, the integration by parts formula \eqref{eq:duality_min} is used.
\end{proof}
...
\alpha_w = \rho h \diffp{w}{t}, \quad \text{Linear momentum}, \qquad \bm{A}_{\kappa} = \bm{\kappa}, \quad \text{Curvature tensor}.	
\end{equation}
The co-energy variables are found by computing the variational derivative of the Hamiltonian:
\begin{equation}\label{eq:e_Kirchh}
e_w := \diffd{H}{\alpha_w} = \diffp{w}{t}, \quad \text{Linear velocity},  \qquad  \bm{E}_{\kappa} := \diffd{H}{\bm{A}_{\kappa}} = \bm{M}, \quad \text{Momenta tensor}.
\end{equation}
...
\begin{theorem}\label{th:adjdivDiv}
	...
	\begin{proof}
		Let $\mathbb{S} = \mathbb{R}^{d \times d}_{\text{sym}}$ and consider the Hilbert space of the square-integrable symmetric square tensors  $L^2(\Omega, \mathbb{S})$ over an open connected set $\Omega$. Consider the Hilbert space $L^2(\Omega)$ of scalar square-integrable functions, endowed with the standard inner product. Consider the double divergence operator defined as: 
		\begin{equation*}
		\begin{aligned}
...
\end{aligned}
\end{equation}
where $\gamma_{0}^{\Gamma_*}a = a\vert_{\Gamma_*}$ and  $\gamma_{1}^{\Gamma_*}a = \partial_{\bm{n}} a \vert_{\Gamma_*}$ denote the trace and the normal derivative trace over the set $\Gamma_*$ respectively. The symbol $\gamma_{nn, 1}^{\Gamma_*}$ denotes the map $\gamma_{nn, 1}^{\Gamma_*}\bm{A} = -\bm{n} \cdot \Div \bm{A} - \partial_{\bm{s}} (\bm{A} \cddot (\bm{n} \otimes \bm{s}))\vert_{\Gamma_*},$, while $\gamma_{nn}^{\Gamma_*}\bm{A} = \bm{A} \cddot (\bm{n} \otimes \bm{n})\vert_{\Gamma_*}$ indicates the normal-normal trace of a tensor-valued function. As the elasticity system \eqref{eq:pHsysElas} and the Mindlin plate \eqref{eq:pHsysMin} the boundary operators $\mathcal{B}_\partial, \mathcal{C}_\partial$ are unbounded. 
...
\begin{remark}[Trace operators and duality pairings for the Kirchhoff plate]\label{rmk:duality_kir}
	...
	\inner{\Hess e_w}{\bm{E}_\kappa} -\inner{e_w}{\div\Div\bm{E}_\kappa} = \inner[H^{\frac{1}{2}} \times H^{-\frac{1}{2}}(\partial\Omega)]{\gamma_{1} e_w}{\gamma_{nn}\bm{E}_\kappa} + \inner[H^{\frac{3}{2}} \times H^{-\frac{3}{2}}(\partial\Omega)]{\gamma_{0} e_w}{\gamma_{nn, 1}\bm{E}_\kappa},
	\end{equation}
...
\begin{theorem}[Stokes-Dirac structure for the Kirchhoff plate]\label{thm:stdirKir}
	...
	V_\partial &:= H^{\frac{1}{2}}(\partial\Omega) \times H^{\frac{3}{2}}(\partial\Omega), \\
	V_\partial' &:= H^{-\frac{1}{2}}(\partial\Omega) \times H^{-\frac{3}{2}}(\partial\Omega). \\
	\end{align*}
	...
	\end{pmatrix}
	\vert \;
	\bm{e} \in H, \quad \bm{f} = \mathcal{J} \bm{e} \in F, \quad \bm{f}_\partial = 
	\begin{bmatrix}
	\gamma_1 \\ \gamma_0 \end{bmatrix} e_w \in V_\partial, \quad \bm{e}_\partial = -\begin{bmatrix}
	\gamma_{nn} \\ \gamma_{nn, 1} \end{bmatrix} \bm{E}_\kappa \in V_\partial'  \right\},
	\end{equation}
	where $\bm{e} = ({e}_w, \, \bm{E}_\kappa)$ and $\mathcal{J}$ is defined in \eqref{eq:pHsysKir}, is a Stokes–Dirac structure with respect to the pairing
...
where
\begin{equation*}
\inner[V_\partial \times V_\partial']{\bm{f}_{\partial}}{\bm{e}_{\partial}} = -\inner[H^{\frac{1}{2}} \times H^{-\frac{1}{2}}(\partial\Omega)]{\gamma_{1} e_w}{\gamma_{nn}\bm{E}_\kappa} - \inner[H^{\frac{3}{2}} \times H^{-\frac{3}{2}}(\partial\Omega)]{\gamma_{0} e_w}{\gamma_{nn, 1}\bm{E}_\kappa}.
\end{equation*}	
	
\end{theorem}
	
\begin{proof}
	The proof is analogous to Theorem \ref{thm:stdirElas}. The integration by parts formula \eqref{eq:duality_kir} is here used.
\end{proof}
...
\end{equation}
Differently from isotropic plate, for laminated anisotropic plates the membrane and bending behavior are coupled. The coupling term $\bm{\mathcal{D}}_{c}$ disappears if a symmetric configuration is considered. For the shear contribution it is obtained
