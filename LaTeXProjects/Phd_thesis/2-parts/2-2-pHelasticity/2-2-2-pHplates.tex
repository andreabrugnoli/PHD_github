...
\begin{equation}
\alpha_w = \mu \diffp{w}{t}, \quad \text{Linear momentum}, \qquad \bm{A}_{\kappa} = \bm{\kappa}, \quad \text{Curvature tensor}.	\end{equation}
...
\begin{theorem}
	...
	\begin{proof}
		...
		\bm{\Psi}& \rightarrow \mathrm{div}(\mathrm{Div}(\bm{\Psi}) = \psi, \\	\end{aligned}
		\qquad \text{with } \psi = \mathrm{div}(\mathrm{Div}(\bm{\Psi}) = \sum_{i = 1}^d \sum_{j = 1}^d \diffp{\bm{\Psi}_{ij}}{x_i,x_j}.
		\]
		We {shall} identify $A^*$
		\[
		\begin{aligned}
...
&= \int_{\Omega} \left\{ - \div(\Div(\bm{E}_{\kappa})) \, e_w + \Grad(\grad(e_w)) \cddot \bm{E}_{\kappa} \right\} \d\Omega, &\qquad \text{Stokes theorem} \\
&=  \int_{\partial \Omega} \left\{ - \bm{n} \cdot \Div(\bm{E}_{\kappa}) \, e_w + \left(\bm{n} \otimes \grad(e_w) \right)\cddot \bm{E}_{\kappa} \right\} \d{s},  \\
&=\int_{\partial \Omega} \left\{ \widehat{q}_n \, e_w + \partial_{\bm{n}}{e_w} \left(\bm{n} \otimes \bm{n} \right) \cddot \bm{E}_{\kappa} + \partial_{\bm{s}}{e_w} \left(\bm{n} \otimes\bm{s} \right)\cddot \bm{E}_{\kappa}   \right\} \d{s}, &\qquad \text{Dyadic properties}\\
...
\cite{timoshenko1959theory}). The tangential derivative has to be moved on the torsional momentum $M_{ns}$. For sake of simplicity, $\partial \Omega$ is supposed to be regular. Then the integration by parts provides
\begin{equation}
...
\end{aligned}
\end{equation}
where $\bm\gamma_{0}^{\Gamma_*}a = a\vert_{\Gamma_*}$ denotes the trace over the set $\Gamma_*$, and  $\gamma_{1}^{\Gamma_*}a = \partial_{\bm{n}} a \vert_{\Gamma_*}$ denotes the trace of the normal derivative over the set $\Gamma_*$. The symbol $\gamma_{nn, 1}^{\Gamma_*}$ denotes the map $\gamma_{nn, 1}^{\Gamma_*}\bm{A} = -\Div \bm{A} \cdot \bm{n} - \partial_{\bm{s}} (\bm{A}(\bm{n} \otimes \bm{s}))\vert_{\Gamma_*},$, while $\gamma_{nn}^{\Gamma_*}\bm{A} = \bm{A} \cddot (\bm{n} \otimes \bm{n})\vert_{\Gamma_*}$ indicates the normal-normal trace of a tensor-valued function.
...
Until now homogeneous isotropic materials have been considered. For this class of materials, the membrane and bending problems are decoupled. In aeronautical applications, structure are made up of layers of different materials to enhance the mechanical properties of the resulting laminated structure.  
