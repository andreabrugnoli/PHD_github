...
Infinite dimensional spaces appears whenever differential operators have to be considered. The fundamental notion of formal adjoint 


\subsection{Differential operators}

Let $\Omega$ denote a compact subset of $\mathbb{R}^d$ representing the spatial domain of the distributed parameter system. Consider two function space $F_1, F_2$ over the fields $\bbA, \; \bbB$ defined on $\Omega \subset \mathbb{R}^d$ and a map $\mathcal{L}$ relating the two
\begin{equation}
	\begin{aligned}
...
Fields $\mathbb{A}, \mathbb{B}$ denote can either  scalars $\bbR$, vectors $\bbR^d$,  tensors $\bbR^{d \times d}$ or a Cartesian product of those. Given $\bm{u} \in F_1, \;  \bm{v} \in F_2$ The map $\mathcal{L}$ is a  differential operator if it can be represented by a linear combination of the derivate of $\bm{u}$
\begin{equation}
\bm{v} = \mathcal{L} \bm{u} \iff \bm{v} := \sum_{|\alpha|=0}^n  \mathcal{P}_{\alpha} \partial^{\alpha} \bm{u},
\end{equation}
where $\alpha := (\alpha_1, \dots , \alpha_n)$ is a multi-index of order $|\alpha| := \sum_{i=1}^n \alpha_i$ and $\partial^{\alpha} := \partial_{x_1}^{\alpha_1} \dots \partial_{x_d}^{\alpha_d}$ is a differential operator of order $|\alpha|$ resulting from a combination of spatial derivatives. $\mathcal{P}_\alpha: \bbA \rightarrow \bbB$ is a constant algebraic operators from field $\bbA$ to $\bbB$. 

\begin{example}[Divergence operator in $\mathbb{R}^d$]
	Given $\bm{u} \in C^\infty(\Omega, \bbR^d), \; v \in C^\infty(\Omega, \bbR)$, where $C^\infty(\Omega, \bbR), \; C^\infty(\Omega, \bbR^d)$ denotes the set of smooth scalar function defined on $\Omega$, the divergence operator in Cartesian coordinate is expressed as
	\begin{equation}
	v = \div \bm{u} = \sum_{i = 1}^d \bm{e}_i^\top \partial_{x_i} \bm{u},
	\end{equation}
...
In this work a very important notion related to a differential operator is the one of formal adjoint.  Consider the spaces of square integrable functions in domain $\Omega$ over the fields $\bbA, \; \bbB$ and denoted by $L^2(\Omega, \bbA), \; L^2(\Omega, \bbB)$. Their inner product are denoted by $\inner[L^2(\Omega, \bbA)]{\cdot}{\cdot}, \inner[L^2(\Omega, \bbB)]{\cdot}{\cdot}$ For scalars $(a,b) \in L^2(\Omega)$, vectors $(\bm{a}, \bm{b})\in L^2(\Omega, \mathbb{R}^d)$ and tensors  $(\bm{A}, \bm{B}) \in L^2(\Omega,\mathbb{R}^{d\times d})$ the $L^2$ inner product is given by 
\begin{equation}
...
\begin{definition}
Let $\mathcal{L}= L^2(\Omega, \mathbb{A}) \rightarrow L^2(\Omega, \mathbb{B})$ be a differential operator and  $\bm{u}_1 \in C_0^\infty(\Omega, \bbA), \; \bm{u}_2(\Omega, \bbB)$ be smooth variables with compact support on $\Omega$. 
The formal adjoint of the differential operator $\mathcal{L}$, denoted by $\mathcal{L}^*= L^2(\Omega, \mathbb{B}) \rightarrow L^2(\Omega, \mathbb{A})$, is defined by the relation
\begin{equation}
\inner[L^2(\Omega, \bbB)]{\mathcal{L}\bm{u}_1}{\bm{u}_2} = \inner[L^2(\Omega, \bbA)]{\bm{u}_1}{\mathcal{L}^*\bm{u}_2}, \qquad \bm{u}_1 \in C_0^\infty(\Omega, \bbA), \; \bm{u}_2 \in C_0^\infty(\Omega, \bbB), 
\end{equation}{}
\end{definition}
This definition represent an extension to generic fields $\bbA, \; \bbB$ of Def. 5.80 in \cite{rogers2004pde} (reported in Appendix \ref{app:math}). 
