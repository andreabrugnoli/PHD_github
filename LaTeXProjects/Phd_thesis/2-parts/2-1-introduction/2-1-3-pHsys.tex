...
Let $\Omega$ denote a compact subset of $\mathbb{R}^d$ representing the spatial domain of the distributed parameter system. Consider two function space $F_1, F_2$ over the sets $\bbA, \; \bbB$ defined on $\Omega \subset \mathbb{R}^d$ and a map $\mathcal{L}$ relating the two
\begin{equation}
	\begin{aligned}
...
Sets $\mathbb{A}, \mathbb{B}$ denote can either  scalars $\bbR$, vectors $\bbR^d$,  tensors $\bbR^{d \times d}$ or a Cartesian product of those. Given $\bm{u} \in F_1, \;  \bm{v} \in F_2$ The map $\mathcal{L}$ is a linear differential operator if it can be represented by a linear combination of derivatives of $\bm{u}$
\begin{equation}
\bm{v} = \mathcal{L} \bm{u} \iff \bm{v} := \sum_{|\alpha|=0}^n  \mathcal{P}_{\alpha} \partial^{\alpha} \bm{u},
\end{equation}
where $\alpha := (\alpha_1, \dots , \alpha_d)$ is a multi-index of order $|\alpha| := \sum_{i=1}^d \alpha_i$ and $\partial^{\alpha} := \partial_{x_1}^{\alpha_1} \dots \partial_{x_d}^{\alpha_d}$ is a differential operator of order $|\alpha|$ resulting from a combination of spatial derivatives. $\mathcal{P}_\alpha: \bbA \rightarrow \bbB$ is a constant algebraic operators from set $\bbA$ to $\bbB$. 
...
In this work a very important notion related to a differential operator is the one of formal adjoint.  Consider the spaces of square integrable functions in domain $\Omega$ over the sets $\bbA, \; \bbB$ and denoted by $L^2(\Omega, \bbA), \; L^2(\Omega, \bbB)$. Their inner product are denoted by $\inner[L^2(\Omega, \bbA)]{\cdot}{\cdot}, \inner[L^2(\Omega, \bbB)]{\cdot}{\cdot}$ For scalars $(a,b) \in L^2(\Omega)$, vectors $(\bm{a}, \bm{b})\in L^2(\Omega, \mathbb{R}^d)$ and tensors  $(\bm{A}, \bm{B}) \in L^2(\Omega,\mathbb{R}^{d\times d})$ the $L^2$ inner product is given by 
\begin{equation}
...
\end{equation} 
The notation $\bm{A}\cddot \bm{B} = \sum_{i, j} A_{ij} B_{ij}$ denotes the tensor contraction. We can now state the definition of formal adjoint.
...
\begin{remark}[Differences between adjoint and formal adjoint]
	...
	\begin{equation*}
	\inner[{L}^2(\Omega, \mathbb{R}^{d})]{\bm{y}}{\grad {x}}
...
\begin{definition}[Formally skew-adjoint operator]
	\label{def:skewoper}
	Let $\mathcal{J}: C^{\infty}(\Omega, \mathbb{F}) \rightarrow C^{\infty}(\Omega, \mathbb{F})$ be a linear differential operator. Notice that the set $\mathbb{F}$ in the domain and co-domain is the same. Then, $\mathcal{J}$ is formally skew-adjoint (or skew-symmetric) if and only if $\mathcal{J} = -\mathcal{J}^*$. \\
	
	If functions with compact support are considered, i.e. $\bm{u}_1, \bm{u}_2 \in  C^{\infty}_0(\Omega, \mathbb{F})$ a formally skew-adjoint operators is characterized by the relation 
	\begin{equation}
...
\end{aligned}
\end{equation}
Given a linear subspace $W \subset B$, its orthogonal complement of $W$ is the set
\begin{equation}\label{eq:orthocom}
W^\perp = \{\bm{v} \in B \vert \bilprod{\bm{v}}{\bm{w}} = 0, \;  \forall \bm{w} \in W\}
\end{equation}
\end{definition} 

A Stokes-Dirac structure is a subset that equals its orthogonal complement with respect to the bilinear product \eqref{eq:bilprod}.
\begin{definition}[Stokes-Dirac structure]
	The subset $D \subset B$, with $B$ defined in \eqref{eq:Bset} is a Stokes-Dirac structure iff 
	\begin{equation}
	D = D^\perp,
	\end{equation}
...
For a subset to be a Stokes-Dirac structures a link between flow and effort variables must hold.  Consider $\bm{f} \in C^\infty(\Omega, \mathbb{F})$ and $\bm \in C^\infty(\Omega, \mathbb{F})$ and te following relation between the two
...
\begin{assumption}[Existence of boundary operators]
	...
	\end{equation}
	where $L^2(\partial\Omega, \bbR^m)$ represent the space of square integrable functions over the boundary, endowed with the inner product
	\begin{equation}
...
\end{align*}
Given the fact that $\bm{e}$ is arbitrary then
\begin{equation*}
...
\begin{remark}[Differences between adjoint and formal adjoint]
	...
	\underbrace{=}_{\text{I.B.P.}} -\inner[{L}^2(\Omega, \mathbb{R})]{\div(\bm{y})}{x},
	\end{equation*}
	for $\bm{y} \in C_0^\infty(\Omega, \mathbb{R}^{3}), \; {x} \in C_0^\infty(\Omega)$ (I.B.P. stands for integration by parts). The definition of the domain of the operators, that requires the knowledge of the boundary conditions, has not been specified. 
\end{remark}
...
\subsection{Constant Stokes-Dirac structures}
 ...
 To have the most suitable definition of Stokes-Dirac structure for flexible systems, the approach adopted in \cite{macchelli2004modelling} is here recovered. \\
...
\subsection{Distributed port-Hamiltonian system}
...
Suppose that operators $\mathcal{J}, \; \mathcal{B}_\partial, \;  \mathcal{C}_\partial$ in Eq. \ref{eq:phsys} verify Ass. \ref{ass:bdoper}. The connection between the concept of Stokes-Dirac structure and dpHs becomes clear if the following port behavior is considered
...
\end{aligned}
\end{equation}
By proposition \eqref{prop:stdir} System \eqref{eq:phsys} under the port behavior \eqref{eq:portbeh_inf} defines a Stokes-Dirac structure.  Since operators $\mathcal{J}, \; \mathcal{B}_\partial, \;  \mathcal{C}_\partial$ in Eq. \ref{eq:phsys} verify Ass. \ref{ass:bdoper}, System \eqref{eq:phsys} is lossless. The energy rate is in fact given by 
...
\begin{itemize}
	...
	e_p \\
	\bm{e}_v \\
	\end{pmatrix} = \bm{e}_v\cdot\bm{n}\vert_{\partial\Omega}e_p\vert_{\partial\Omega}, \qquad \mathcal{C}_\partial \begin{pmatrix}
	e_p \\
	\bm{e}_v \\
