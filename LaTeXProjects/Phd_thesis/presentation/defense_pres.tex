...
\begin{enumerate}
	...
	\item \onslide<4->{A Galerkin method is employed to obtain a finite-dimensional system. For the approximation basis the Finite Element Method FEM (large sparse matrices) is here employed but Spectral Methods SM (small full matrices) can be used as well.}
\end{enumerate}
\end{frame}
...
The Hamiltonian is a non-linear and non-separable functional
\begin{equation*}
...
\begin{equation*}
\begin{aligned}
\inner[L^2(\Omega)]{v_w}{\rho b \partial_t e_w} &= -\inner[L^2(\Omega, \mathbb{R}^{2})]{\grad v_w}{\bm{e}_\gamma} + \inner[L^2(\Omega)]{v_w}{f}, \\
...
\begin{block}{CG elements}
	\only<1>{
		...
		H_{0, h, \text{CG}}^{\Grad}(\Omega, \bbR^2) &= \{ \bm{\theta}_h \in H_0^{\Grad}(\Omega, \bbR^2) \vert \ \forall Q \in \mathcal{R}_h,\ \bm{\theta}_h|_{Q} \in (\mathrm{CG}_{k})^2 \}, \\
...
\begin{tcolorbox}
	\centering
	\only<1>{ \includegraphics[width=0.9\textwidth]{part_4/validation/KP/plate_rod_separated.eps}}
	\only<2>{\includegraphics[width=0.7\textwidth]{part_4/validation/KP/plate_rod_welded.eps}}
\end{tcolorbox}
...
\centering \includegraphics[width=0.6\textwidth]{part_4/validation/KP/pp_interconnection.eps}
\end{frame}
...
\begin{itemize}
	...
	\item \onslide<6>{Control: observer based strategies\footfullcite{toledo2020}.}
\end{itemize}
\end{frame}
...
\only<2>{\vspace{.5cm}\centering \includegraphics[width=0.6\textwidth]{part_4/validation/KP/pp_interconnection.eps}}
