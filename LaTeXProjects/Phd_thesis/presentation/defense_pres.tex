...
The $\Hess$ operator is integrated by parts twice to enforce weekly the Neumann conditions. The Dirichlet condition is imposed weakly through a Lagrange multiplier (strong imposition of boundary conditions for $H^2$ conforming elements is not trivial \footfullcite{kirby2019}). 
...
For this case, a simple element is the one proposed in Becaché 2001 \footfullcite{becache2001elas}. This element is constructed as a tensor product on interval elements. \\

\begin{block}{BJT element}
\only<1>{
Let $\mathcal{R}_h$ be a regular mesh with square elements $Q$. The following spaces are introduced as discretization spaces
\begin{equation*}
\label{eq:BTJ}
\begin{aligned}
...
\begin{figure}
\centering
\includegraphics[width=0.95\textwidth]{presentation/fe_BJT.eps}
