...
\only<1>{
Wave-like equations (e.g. linear elastic models) possesses a peculiar structure that can be deduced from their primal (or classical) formulation \footfullcite{joly2003variational}.
...
Assume that there exists  two boundary operators $\mathcal{N}_{\partial, 1}, \; \mathcal{N}_{\partial, 2}$ such that  a general integration by parts formula holds
\begin{equation*}
...
\begin{frame}{The general discretized system}
...
\end{equation*}
By integrating by parts  $-\mathcal{L}^*$ to obtain the appropriate causality.
\begin{exampleblock}{Finite dimensional system}
\begin{equation*}
\begin{aligned}
\begin{bmatrix}
