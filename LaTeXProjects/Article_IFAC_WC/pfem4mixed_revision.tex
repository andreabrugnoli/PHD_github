...
\begin{aligned}
\begin{aligned}
p &\approx \sum_{i=1}^{n_p} \phi_p^i p^i, \\
\bm{v} &\approx \sum_{i=1}^{n_v} \bm\phi_v^i v^i, \\
\end{aligned} \qquad
\begin{aligned}
*_D &\approx \sum_{i=1}^{n_D} \phi^i_\Gamma *^i_D, \quad (* = \{u, y, \lambda\}), \\
*_N &\approx \sum_{i=1}^{n_N} \phi_\Gamma^i *^i_N, \quad (* = \{u,  y\}),
\end{aligned} 
\end{aligned}
\end{equation}
...
\begin{align*}
m^{\Omega_N}(w, \partial_t{e}) &:= \int_{\Omega_N} w \cdot \mathcal{Q}^{-1} e \ \d{\Omega_r}, \\
m^{\Omega_D}(w, \partial_t{e}) &:= \int_{\Omega_D} w \cdot \mathcal{Q}^{-1} e \ \d{\Omega_r},  \\
...
\begin{itemize}
    \item $\phi_p$ is interpolated using order~1 Lagrange polynomials;
    \item $\bm{\phi}_v$ is interpolated using order~2 Raviart-Thomas polynomials;
    \item ${\phi}_\Gamma$ is approximated by Lagrange polynomial of order~1 defined on the boundary $\Gamma_D$ (for $\lambda_D, u_D, y_D$) or $\Gamma_N$ (for $u_N, y_N$).
\end{itemize}
