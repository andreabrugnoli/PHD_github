...
\end{quotebox}
The considered numerical examples represent a validation of the proposed formulation with respect to preexisting publications. It is indeed difficult to find in the literature benchmark examples on the floating frame formulations.
...
\end{quotebox}
In principle this formulation works with any kind of joints. There are however many issues that require further investigations. 
\begin{enumerate} 
...
\begin{tcolorbox}[title=Revision page 25 (Reviewer No. 1 -- Comment h ),colframe=colorRevBG1]
...
\end{quotebox}
No stabilization is used. Imposing the constraints at the velocity level allows preserving the overall pH structure (since the power flowing is computed through the velocities). T
