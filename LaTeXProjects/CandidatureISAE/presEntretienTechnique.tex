...
\begin{frame}{Poste et motivation}
	\centering
		\begin{tcolorbox}[title = Missions, coltitle=white, hbox]
			\begin{tabular}{c|c}
...
\begin{frame}{Encadrement projets et thèses}
	\textbf{Expériences} : 
	\begin{itemize}
		\item Organisation et encadrement du PIE : "Simulation et contrôle des structures thermoélastiques pour applications spatiales".
		\item Supervision du PFE de Vitor Borges Santos, en double diplôme ITA/UT Twente : "Réduction de modèle pour structures minces très flexibles" (collaboration avec prof. Flavio Cardoso Ribeiro)
		\item Supervision de la thèse "On the modeling and mechanical design of flexures (compliant mechanisms)" (collaboration avec prof. Marijn Nijenhuis).
	\end{itemize}
...
\begin{frame}{Articles de revue (mathématiques appliques)}
\begin{enumerate}
...
\begin{frame}{Congres internationales (mathématiques appliques)}
\footnotesize	
\begin{enumerate}
...
\section{Détails Activités recherches}

\begin{frame}{Activités recherche post Doc}
	Discrétisation des systèmes port-Hamiltoniens en utilisant les éléments finis en calcul extérieur.
\end{frame}
