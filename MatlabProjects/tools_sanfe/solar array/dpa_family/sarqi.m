function [poles, rightev, leftev, nr_solves, ress] = ...
           sarqi(A, E, b, c, d, s0, options)
% [poles, leftev, rightev, nr_solves] = ...
%     sarqi(A, E, b, c, d, s0, options)
% computes the right-most poles or poles close to damping ratio of SISO transfer function
%               H(s) = c'*(sE-A)^{-1}b + d
% of the system
%               E * x' = A*x    + b*u
%                   y  = c' * x + d*u
% using the Subspace Accelerated Rayleigh Quotient Algorithm of Joost Rommes and
...
%    IEEE Transactions on Power Systems, Vol. 25, Issue 2, May 2010, pp.929-938
%
% Input:
...
%                           'DR' : see also below at damping_ratio
...
%
%         Advanced options:
%         damping_ratio: if strategy='DR', select poles closest to damping ratio (0)
%                            if damping_ratio is a list, nwanted eigenvalues per
%                             per ratio will be computed. Damping ration of -1 gives
%                             for real rightmost, vary ratio between -1 and 1 to
%                             scan complex plane. See paper for details.
...
%
% Note this implementation has several todos so search for 'todo'!
%
% Joost Rommes (C) 2008--2010
% rommes@gmail.com
% http://sites.google.com/site/rommes

fprintf( 'Note this implementation has several todos so search for todo!\n') ;

if nargin < 7
    options = struct([]) ;
end

[n, nwanted, tol, strategy, damping_ratio_list, use_lu, use_lu_w_amd, kmin, krestart, maxrestarts, ...
...
nr_shifts = size(s0,1) ;
shifts(:,1) = s0 ; %pole approximations
shift_nr = 1 ;
just_found = 0 ;
...
schurvec = rand(n, 1); schurvec = schurvec / norm(schurvec) ;
lschurvec = rand(n,1) ; lschurvec = lschurvec / norm(lschurvec) ;
schurvec = (1:n)'; schurvec = schurvec / norm(schurvec) ;
lschurvec = (1:n)' ; lschurvec = lschurvec / norm(lschurvec) ;

%                [s,l,t]=twosided_rqi(A, E, f_ax, f_ex, f_semax, f_semax_s, ...
%                             schurvec, lschurvec, st, tol, ...
%                             imagtol, use_lu, 1, dpa_bordered) ; t
nwanted_dr = nwanted / length(damping_ratio_list) ;
while nrestart < maxrestarts
    damping_ratio = damping_ratio_list(1+floor(nr_converged/nwanted_dr)) ;
...
%    b = E *  schurvec / norm(schurvec) ; %todo fix for function refs
%    c = E'* lschurvec / norm(lschurvec);
    if b_f_ex == 0
        Ex = E*schurvec ;
        Ey = E'*lschurvec ;
    else
    ...
    if just_found && shift_nr < nr_shifts
        just_found = 0 ;
        st = shifts( shift_nr, 1) ;
        shift_nr = shift_nr + 1 ;
    end
    
    newvector = 1 ;
    while newvector %todo
    if dpa_bordered %original dpa form
        ...
        if b_f_semax_s == 0
            ...
            x = f_semax_s( 'N', b, st, 0 ) ;
            v = f_semax_s( 'T', c, st, 0) ;
        end
    end
...
%    h(end), Qtsparse'*Q,  nx = norm(X), pause
    if abs(h(end)) > 1e-10 %&& norm( Qtsparse'*x/norm(x) ) < 1e-14
    newvector = 0 ;
    X(:, k) = x / h(end) ;
%Qtsparse'*X, pause %todo
    [v,h] = mgs(V, v,1) ;
	V(:, k) = v / h(end) ;
    else
%    h(end)
    st = rand(1) + rand(1) * 10 * sqrt(-1) ;
    end %endif h < 
    end %while
    ...
    [SH, SHt, UR, URt] = select_max_res( H, G, X, V, b, c, strategy, damping_ratio, E, f_ex, yEx_scaling ) ;
    SG = eye(size(SH)) ;
        ...
        if found
            ...
            if abs(imag( cce )) / abs(cce) >= imagtol %relative
                ...
                if 1 %min( pairdifs ) > tol
                   ...
                   if b_f_semax == 0
                       r = A*ccv - cce*(E*ccv) ;
                       r = lschurvec'*A - theta*lschurvec'*E ; %todo
                   else
                       r = f_semax( 'N', ccv, cce ) ;                   
                   end
                   if displ
            ...
            J = 2 : k ; %todo remove again?
            X = X(:, J) ; 
            V = V(:, J) ;
            k = k -1 ;
%            qtq = norm(Qtsparse'*Q) %, pause
...
function [SH, SHt, UR, URt] = select_max_res( H, G, X, V, b, c, strat, damping_ratio, E, f_ex, yEx_scaling )
...
for i = 1 : n
    ...
    damping_rat(i) = damping_rat(i) - damping_ratio ;
    
    if abs(real(dd(i))) == Inf || abs(imag(dd(i))) == Inf || abs(real(ddt(i))) == Inf || abs(imag(ddt(i))) == Inf
        fprintf( 'Numerical issues with computing damping ratio -- check your data!\n' ) ;
        damping_rat(i) = 1000 ;
    end
%    residue(i) = 1 ;
%    if abs( V(:,i)'*(E*X(:,i)) ) < 1e-8
%        ddt(i) = -1/abs(ddt(i)) ;
%    end
%    residue(i) = abs( V(:,i)'*(E*X(:,i)) ) ;
...
if isnumeric( strat )
	...
	switch strat
		...
		case 'DR'
            residue = -abs(damping_rat) ;
        case 'LR'  
			if 0
                residue = residue ./ real(ddt.') ;
%                residue = residue .* (real(ddt.') ./ abs(ddt.')) ;%residue ./ real(ddt.') ;
            else
                for i = 1 : length(residue)
                    if real(ddt(i)) > 0
                        residue(i) = residue(i) ./ real(ddt(i).') ;
                    else
                        residue(i) = residue(i) * real(ddt(i)) ;
                    end
                end
...
function [n, nwanted, tol, strategy, damping_ratio, use_lu, use_lu_w_amd, kmin, kmax, maxrestarts, displ, dpa_bordered, yEx_scaling, rqitol, tdefl, f_ax, f_ex, f_semax, f_semax_s]  = init( A, options )
...
if isfield( options, 'damping_ratio' ) %target
    damping_ratio = options.damping_ratio ;
    nwanted = nwanted*length(damping_ratio) ;
else
    damping_ratio = 0 ;
end
...
while nrq > tol && iter < maxit
    ...
    if b_f_semax_s == 0
        ...
        x_rqi = f_semax_s( 'N', Ex, theta, 1 ) ;
        v_rqi = f_semax_s( 'T', Ey, theta, 1 ) ;
    end
    nr_solves = nr_solves + 1 ;
    ...
    rqi_res_l = 0 ; % v_rqi'*A - x_rq * v_rqi'*E ; %todo
    ...
    y = v_rqi ;
    theta = x_rq ;
%    nrq = norm(rqi_res) %todo
    nrq = max( norm(rqi_res), norm(rqi_res_l) ) 
    if ~isfinite( nrq )
        nrq = 1 ;
    end
end
...
if ogs
    ...
    if norm(u) < 1e-8 %|| nqqu > 1e-14 %todo
