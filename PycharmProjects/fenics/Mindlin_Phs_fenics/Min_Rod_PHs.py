...
f_w =Expression("10000*( pow((x[1]-l_y/2), 2) + x[1]/10)", degree= 4, l_y = l_y) #
...
M_rod = np.array([[m_rod, 0], [0, Jxx_rod]])
invM_rod = la.inv(M_rod)
...
p0_pl = Expression(('sin(2*pi*x[0])*sin(2*pi*(x[0]-lx))*sin(2*pi*x[1])*sin(2*pi*(x[1]-ly))', '0', '0'), degree=4, lx=l_x, ly=l_y)
...
for i in range(1,n_t + 1):
    ...
    lmbda = invM_lambda @ (-GT @ invMp @ (D_q @ eq_pl_old + B_p * f))

    bp_rod = M_rod @ ep_rod_old - 0.5 * dt * np.concatenate((np.zeros((2, n_l)), C), axis=1) @ lmbda
    ep_rod_new = invM_rod @ bp_rod

    bp_pl = M_p @ ep_pl_old + 0.5 * dt * (D_q @ eq_pl_old + G @ lmbda + B_p * f)
    ...
    bq_pl = M_q @ eq_pl_old + dt * D_p @ ep_pl_new
    ...
    lmbda = invM_lambda @ (-GT @ invMp @ (D_q @ eq_pl_new + B_p * f))

    bp_rod = M_rod @ ep_rod_old - 0.5 * dt * np.concatenate((np.zeros((2, n_l)), C), axis=1) @ lmbda
    ep_rod_new = invM_rod @ bp_rod

    bp_pl = M_p @ ep_pl_old + 0.5 * dt * (D_q @ eq_pl_new + G @ lmbda + B_p * f)
    bp_pl_sp = csr_matrix(bp_pl).reshape((n_Vp, 1))
    ep_pl_new = spsolve(Mp_sp, bp_pl_sp)
...
for i in range(n_ev):
    Hpl_vec[i] = 0.5  * (ep_pl_sol[:,i].T @ M_p @ ep_pl_sol[:,i] + eq_pl_sol[:,i].T @ M_q @ eq_pl_sol[:,i])
    Hrod_vec[i] = 0.5  * (ep_rod_sol[:,i].T @ M_rod @ ep_rod_sol[:,i])
