...
P_wt = FiniteElement('CG', triangle, deg)
P_omn = FiniteElement('CG', triangle, deg)
P_oms = FiniteElement('CG', triangle, deg)

element_u = MixedElement([P_wt, P_omn, P_oms])

Vu = FunctionSpace(mesh, element_u)

wt, om_n, om_s = TrialFunction(Vu)
...
for key,val in bc_dict.items():
    if val == 'F':
        b_vec.append( v_qn * wt * ds(key) + v_Mnn * om_n * ds(key) + v_Mns * om_s * ds(key))
    elif val == 'S':
