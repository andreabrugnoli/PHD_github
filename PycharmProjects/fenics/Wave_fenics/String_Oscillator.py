...
Pp = FiniteElement('DG', mesh.ufl_cell(), deg-1)
...
for i in range(1,n_t + 1):
    ...
    bp_st = M_p @ ep_st_old + 0.5 *dt * D_q @ eq_st_old
    bp_sp = csr_matrix(bp_st).reshape((n_Vp,1))

    ep_st_new = spsolve(Mp_sp, bp_sp)

    ep_os_new = 1./((m + 0.5*dt*r)) * (m * ep_os_old - 0.5 * dt*( BT @ eq_st_old + eq_os_old))

    ep_st_old = ep_st_new
    ep_os_old = ep_os_new
    # Integration of q (n+1)

    bq_st = M_q @ eq_st_old + dt * (D_p @ ep_st_new + B * ep_os_new)
