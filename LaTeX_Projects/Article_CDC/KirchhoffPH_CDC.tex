...
\[ \bm{P}_{\lambda} = \bm{I} - \bm{G} \left(\bm{G}^T \bm{M}^{-1} \bm{G} \right) \bm{G}^T \bm{M}^{-1}
\]
...
The damping injection technique allows to asymptotically stabilize and infinite dimensional system. It relies on the La~Salle invariance principle (\cite{bookPHs}  chapter 6, proposition 6.2). Starting from system \eqref{eq:discr_pl}
