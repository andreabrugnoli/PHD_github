...
\begin{equation*}
\mathrm{Hess}(\bm{u}) = \mathrm{Grad}(\mathrm{grad}(u)),
\end{equation*}
...
\end{equation*}
Furthermore, $\mathbb{S}$ denotes the space of symmetric $d \times d$ matrices. The geometrical dimension of interest in this paper is $d=2$. In the following vectors (tensor) fields and numerical vectors (matrices) will be denote by a lower (upper) case bold letter. It will be clear by the context which mathematical objects is considered.
...
\begin{gathered}
\left.\begin{aligned}
\bm{G}_{w}(i,j) &= \int_{\Gamma_{C} \cup \Gamma_{S} } {\phi}_{w}^i \, {\phi}_{\widetilde{q}_n}^j \d{s}, \\
\bm{B}_{\widetilde{q}_n}(i,j) &= \int_{\Gamma_{\widetilde{q}_n}} {\phi}_{w}^i \, {\phi}_{\widetilde{q}_n}^j \d{s},  \\
\end{aligned} \; \right\} \quad  \in \mathbb{R}^{N_w \times N_{q_n}}, \\
\left. \begin{aligned}
\bm{G}_{\omega_n}(i,j) &= \int_{\Gamma_{C}} \diffp{\phi_w^i}{n} \, \phi_{m_{nn}}^j \d{s}, \\
\bm{B}_{m_{nn}}(i,j) &= \int_{\Gamma_{m_{nn}}} \diffp{\phi_w^i}{n} \, \phi_{m_{nn}}^j \d{s}, \\
\end{aligned} \; \right\} \quad \in \mathbb{R}^{N_w \times N_{m_{nn}}},
\end{gathered}
\end{equation}
where $\Gamma_{C},\Gamma_{S}$ are subsets of the boundary where clamped and simply supported boundary conditions apply and $\Gamma_{\widetilde{q}_n},\Gamma_{m_{nn}}$ are subset Consequently, the input matrix reads
...
\end{remark}
The Firedrake library \cite{firedrake} was used to implement the numerical analysis as it provides functionalities to automate the generalized mappings for $H^2$ conforming finite elements (like the Hermite, Bell or Argyris finite elements). \\ 
...
\section{Interconnection with a finite dimensional pH system}
In this section the interconnection of an infinite and finite port system is explained in 
