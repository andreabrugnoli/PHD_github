...
acts on the plate. The rigid rod has mass $M = 50 \, [kg]$ and length $L_{\text{rod}} = 1 [m]$. The plate parameters and settings for the discretization (a uniform grid is taken) are provided in table \ref{tab:par}. The constraints are classically eliminated considering the projection matrix
...
\label{fig:H_rod}}
\caption{Hamiltonian trend for the interconnection of a cantilever plate with a rigid rod. For comparison in \ref{fig:H_Norod} the same simulation is performed without including the rod}
\label{fig:HamInt}
\end{figure}
...
\begin{figure*}[t]
	...
	\subfloat[$t=0.32 \, t_{\text{end}}$]{\includegraphics[width=0.25\linewidth]{Snapshot_t320_cropped.eps}%
		...
		\subfloat[$t=0.965 \, t_{\text{end}}$]{\includegraphics[width=0.25\linewidth]{Snapshot_t965_cropped.eps}%
			\label{fig:Damp_8}}
		\hfil
	\caption{Snapshots at different times for the simulation of the boundary controller by damping injection ($t_{\text{end}} = 10 \,[ms]$). The plate is clamped at $x = 0$ and the controller acts on the rest of the boundary $\Gamma_{\text{control}} = \left\{(x,y) \vert\; x=L_x \cup y=0 \cup y = L_y \right\}$.}
	\label{fig:SnapDamp}
	\hfil
\end{figure*}
...
\addtolength{\textheight}{-4.7cm}   % This command serves to balance the column lengths
