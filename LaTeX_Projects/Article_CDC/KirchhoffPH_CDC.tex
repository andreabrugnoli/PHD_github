...
In this section the classical formulation of the Kirchhoff plate the tensorial pH formulation are recalled. 
...
\end{aligned} 
\end{equation}
Here the Lagrange multipliers and associated matrices $\bm{\lambda}_D =  (\bm\lambda_{\Gamma_{C}}; \; \bm\lambda_{\Gamma_{\text{int}}} ), \; \bm{G}_D = [\bm{G}_{\Gamma_{C}}, \; \bm{G}_{\Gamma_{\text{int}}}], \; \bm{B} = [\bm{0}; \; \bm{I}]$ are split between the homogeneous and non-homogeneous Dirichlet boundary conditions. The rigid rod is written compactly 
\begin{equation}
\begin{aligned}
...
To validate the approach numerical simulations on system \eqref{eq:aug_Int} are carried out. A  plate clamped in $x=0$ is considered, i.e. $\Gamma_C = \{(x,y) \vert x = 0\}$. A final time equal to $t_{\text{end}} = 10 [ms]$ is taken and a vertical distributed force given by expression
\[
\bm{f}_w = \begin{cases}
...
\begin{figure}[t]
\centering
\subfloat[Without rod]{\includegraphics[width=0.5\linewidth]{HamiltonianNoRod_cropped.eps}%
\label{fig:H_Norod}}
\hfil
\subfloat[With rod]{\includegraphics[width=0.5\linewidth]{HamiltonianRod_cropped.eps}%
\label{fig:H_rod}}
...
Snapshots of simulations without and with the rigid rod are reported in figures \ref{fig:IntNoRod}, \ref{fig:IntRod}. The deformations undergone by the plate are clear affected by the presence of the rod: the maximum deformation as well as the Hamiltonian value, once the excitation is removed, are lower. The interconnected side remains straight along the whole simulation, meaning that the constraints are respected.
...
The damping injection technique allows to asymptotically stabilize and infinite dimensional system. It relies on the La~Salle invariance principle (\cite{bookPHs}  chapter 6, proposition 6.2). Starting from system \eqref{eq:discr_pl}, consider a cantilever plate and the following static control law
\begin{equation}
\bm{u} = -\bm{K} \bm{y}.
\end{equation}
...
Videos are accessible at \url{https://github.com/brugnoss/Goodies_pH_plates}
\begin{figure}[t]
	\centering
...
\begin{figure}[t]
\centering
\includegraphics[width=0.8\linewidth]{HamiltonianDamped_cropped.eps}
\caption{Hamiltonian trend for }
\label{fig:HamDamped}
\end{figure}
