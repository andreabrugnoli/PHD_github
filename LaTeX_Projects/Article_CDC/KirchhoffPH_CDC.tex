...
\subsection{General Ideas for the infinite dimensional setting}
...
\begin{minipage}{.5\linewidth}
	...
	\bm{y}_{2} = \bm{C} \diffp{H_2}{\bm{x}_2} + \bm{D} \bm{u}_2 \\
	\end{aligned},
	\right. 
	\end{equation}
\end{minipage} %
...
\paragraph{Example 1: Rigid rod welded to the plate}
A rigid rod can be written undergoing small displacements about the $z$ axis and small rotation about the $x$ axis is a port-Hamiltonian system with structure
\begin{equation}
\begin{aligned} \dot{\bm{x}}_{\text{rod}} =
\displaystyle \diff{}{t}
\begin{pmatrix}
p_w \\ p_\theta \\
...
\end{pmatrix} = \bm{u}_{\text{rod}}\vspace{1mm} \\
 ...
 v_G \\ \theta_{G} \\
 \end{pmatrix} = \displaystyle \diffp{H_{\text{rod}}}{\bm{x}_{\text{rod}}}
\end{aligned}, 
\end{equation}
with $p_w, p_\theta$ the linear and angular momentum about the center of mass, $v_G, \theta_{G}$ the linear and angular velocity about the center of mass $G$ and $F_z, T_x$ the force along $z$ and the torque along $x$. The Hamiltonian reads $H_{\text{rod}}  = \frac{1}{2} \left( \frac{p_w^2}{M_G} + \frac{p_\theta^2}{J_G}\right)$, with $M, J_G$ the mass and rotary inertia about the $x$ axis. The rod is welded to a rectangular thin plate of sides $L_x, L_y$ on side $x = L_x$. The boundary variables for the plate involved in the interconnection are 
\[u_{\partial, \text{pl}} = w_t\vert_{x = L_x}  \qquad  y_{\partial, \text{pl}} = \widetilde{q}_n\vert_{x = L_x}
\]
The space $\mathscr{Y}$ is the space of square integrable functions on with support $\partial\Omega_{\text{int}} = \left\{ (x,y) \vert \; x=L_x, 0 \le y \le L_y  \right\}$ compact interconnection operator then reads
\begin{equation}
	\mathcal{I} \widetilde{q}_n\vert_{x = L_x} \begin{pmatrix}
	\int_{y = 0}^{y = L_y} \widetilde{q} \d{y} \\
	\int_{y = 0}^{y = L_y} \widetilde{q} \left( y - L_y/2 \right) \d{y} \\
	\end{pmatrix} 
\end{equation}
