...
\end{aligned} 
\end{equation}
where $\Gamma_{C},\Gamma_{S}$ are subsets of the boundary where clamped and simply supported boundary conditions apply and $\Gamma_{\widetilde{q}_n},\Gamma_{m_{nn}}$ are subsets where inhomogeneous Neumann conditions hold. The final port-Hamiltonian descriptor system (pHDAEs), as defined in \cite{beattie2018linear}, is written as
\begin{equation}
\label{eq:discr_pl}
\begin{bmatrix}
\bm{M}_{\text{pl}} & \bm{0} \\
...
\bm{f}_N,
\end{equation}
where $\bm{e}_{\text{pl}} = (\bm{e}_w; \, \bm{e}_\kappa), \;  \bm{\lambda}_D = (\bm{\lambda}_{\widetilde{q}_n}; \, \bm{\lambda}_{m_{nn}}), \; \bm{f}_N = (\widetilde{\bm{q}}_n; \, \bm{m}_{nn})$ are column-wise (denoted by $;$) concatenation of the co-energy variables, Lagrange multipliers and boundary forces and momenta at the borders and 
...
\[
\left\langle \mathcal{W} y_{\partial, \text{pl}}, \; \bm{y}_{\text{rod}} \right\rangle_{\mathbb{R}^m} = \left\langle \mathcal{W}^* \bm{y}_{\text{rod}} , \; y_{\partial, \text{pl}} \right\rangle_{L^2(\partial\Omega_{\text{int}})},
\]
\[
...
\begin{equation}
\begin{aligned}
\bm{M}_{\text{rod}} \dot{\bm{e}}_{\text{rod}} = \bm{u}_{\text{rod}}, \\
...
\begin{equation}
\bm{E}_{\text{aug}} \dot{\bm{x}}_{\text{aug}} = \bm{J}_{\text{aug}}\bm{x}_{\text{aug}}, 
\end{equation}
...
To validate the approach numerical simulations considering system \eqref{eq:aug_Int} were carried out. A final time equal to $t_{\text{end}} = 10 [ms]$ is taken. A vertical distributed force given by expression
\[
\bm{f}_w = \begin{cases}
