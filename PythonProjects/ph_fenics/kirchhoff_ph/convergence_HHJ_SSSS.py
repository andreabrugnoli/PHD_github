...
def compute_err(n, r):
    ...
    dxx_wex = Expression("- pow(pi/a,2)*sin(pi*x[0]/a)*sin(pi*x[1]/b)*sin(beta*t)", degree=degr, beta=beta, a=Lx, b=Ly, t=t)
    dyy_wex = Expression("- pow(pi/b,2)*sin(pi*x[0]/a)*sin(pi*x[1]/b)*sin(beta*t)", degree=degr, beta=beta, a=Lx, b=Ly, t=t)
    dxy_wex = Expression("pow(pi,2)/(a*b)*cos(pi*x[0]/a)*cos(pi*x[1]/b)*sin(beta*t) ", degree=degr, beta=beta, a=Lx, b=Ly, t=t)
    ...
    force1 = Expression("sin(pi*x[0]/a)*sin(pi*x[1]/b)*sin(beta*t)*"
                       "(D*pow( pow(pi/a, 2) + pow(pi/b, 2) , 2) - pow(beta,2)*rho*h)",
                       degree=degr, beta=beta, a=Lx, b=Ly, t=t, D=D, rho=rho, h=h)
