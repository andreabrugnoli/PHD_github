...
def print_modes(Mmat, Jmat, Vp, n_modes):
    ...
    tab_coord = Vp.tabulate_dof_coordinates().reshape((-1, 2))
    x_cor = tab_coord[:, 0]
    y_cor = tab_coord[:, 1]
    ...
    for i in range(int(n_modes)):
        ...
        if norm_imag_eig > norm_real_eig:
            tri_surf = ax.plot_trisurf(x_cor, y_cor, eig_imag_p, cmap=cm.jet, linewidth=0, antialiased=False)
        else:
            tri_surf = ax.plot_trisurf(x_cor, y_cor, eig_real_p, cmap=cm.jet, linewidth=0, antialiased=False)
