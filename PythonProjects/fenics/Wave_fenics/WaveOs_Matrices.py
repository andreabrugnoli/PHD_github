...
savepathMatlab ="/home/a.brugnoli/MatlabProjects/DAE_solution/WaveEq/"
savepathPython ="/home/a.brugnoli/PycharmProjects/SolveDAEs/"
savepathJulia ="/home/a.brugnoli/JuliaProjects/TestDAE/"

savepath = savepathJulia

J_file = "J"; M_file = "M"; R_file = "R"; X0_file = "X0";
dofsV_xFile = 'dofsX'; dofsVpFile = 'dofsVp'; dofsVqFile = 'dofsVq';
partFile = 'partitioning';

part = np.array([end1, end2, end3])

np.save(savepath + J_file, Jint)
np.save(savepath + M_file, Mint)
np.save(savepath + R_file, Rint)
np.save(savepath + dofsV_xFile, dofV_x)
np.save(savepath + dofsVpFile, dofs_Vp)
np.save(savepath + dofsVqFile, dofs_Vq)
np.save(savepath + X0_file, y0)
np.save(savepath + partFile, part)



# from scipy.io import savemat
#
# savemat(savepath + J_file, mdict={J_file: Jint})
# savemat(savepath + M_file, mdict={M_file: Mint})
# savemat(savepath + R_file, mdict={R_file: Rint})
# savemat(savepath + X0_file, mdict={X0_file: y0})
# savemat(savepath + dofsV_xFile, mdict={dofsV_xFile: dofV_x})
# savemat(savepath + dofsVpFile, mdict={dofsVpFile: dofs_Vp})
# savemat(savepath + dofsVqFile, mdict={dofsVqFile: dofs_Vq})
# savemat(savepath + partFile, mdict={partFile: np.array([end1, end2, end3])})


#
# def sys(t, u, du):
#     # e_st = y[:end1]; lmb = y[end1: end2]; e_os = y[end2: end3]
#     # de_st = yd[:end1]; dlmb = yd[end1: end2]; de_os = yd[end2: end3];
#     #
#     # # de_st = invMst @ (Jst @ e_st + G @ lmb)
#     # # de_os = invMos @ ((Jos - Ros) @  e_os- C_int.T @ lmb)
#     #
#     # res_0 = Mst @ de_st - Jst @ e_st - G @ lmb
#     #
#     # res_1 = -GT @ (invMst @ (Jst @ (invMst @ (Jst @ e_st + G @ lmb)) + G @ dlmb)) \
#     #         + C_int @ (invMos @ ((Jos - Ros) @ (invMos @ ((Jos - Ros) @  e_os- C_int.T @ lmb)) \
#     #                              - C_int.T @ dlmb)) # First Solution very naive
#     # res_2 = Mos @ de_os - (Jos - Ros) @ e_os + C_int.T @ lmb
#     # return np.concatenate((res_0, res_1, res_2), axis=0)
#     return Mint @ du - (Jint - Rint) @ u
#
#
#
# from assimulo.problem import Implicit_Problem
# from assimulo.solvers import GLIMDA as solver
#
# n_ev = 100
# t_ev = np.linspace(t_0, t_f, n_ev)
#
...
# t_0 = 0
# t_f = 10
# model = Implicit_Problem(sys, y0, yd0, t_0)  # Create an Assimulo problem
# model.name = 'Wave_Int_PHs'  # Specifies the name of problem (optional)
#
# algvar = np.zeros((n_tot,) )
# algvar[:end1] = 1; algvar[end2: end3] = 1;
# sim = solver(model)  # Create the IDA solver
# sim.algvar = algvar
# sim.atol = 1.0e-6
# sim.rtol = 1.0e-6
...
#
#
# t, sol, sold = sim.simulate(t_f, ncp = 0, ncp_list = t_ev)
#
# dt_vec = np.diff(t)
# n_ev  = len(sol[:,1])
# e_st = sol[:, :end1].T
