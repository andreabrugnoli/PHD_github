...
n_V = V.dim()
n_Vp = Vp_w.dim() + Vp_th.dim()
n_Vq = Vq_th.dim() + Vq_w.dim()

plate = SysPhdaeRig(n_V, 0, 0, n_Vp, n_Vq, E=MM, J=JJ, B=B_in)
...
for i in range(n_fig):
    ...
    eig_real_pw = np.real(eigvec_i[:n_Vpw])
    eig_imag_pw = np.imag(eigvec_i[:n_Vpw])
