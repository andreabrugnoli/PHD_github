...
def create_sys2(mesh2, r2):
    ...
    qn_2 = TrialFunction(Vu)
    Mnn_2 = TrialFunction(Vu)
    Mns_2 = TrialFunction(Vu)

    vomn_2 = dot(vpth_2, n_ver2)
    voms_2 = dot(vpth_2, s_ver2)

    is_int2 = conditional(Or(gt(y2, x2 - tol), lt(y2, L - x2 + tol)), 1, 0)

    b_N1 = vpw_2 * qn_2 * is_int2 * ds2
    b_N2 = vomn_2 * Mnn_2 * is_int2 * ds2
    b_N3 = voms_2 * Mns_2 * is_int2 * ds2
    ...
    int_dofs2 = np.where(B_N1.any(axis=0))[0]
