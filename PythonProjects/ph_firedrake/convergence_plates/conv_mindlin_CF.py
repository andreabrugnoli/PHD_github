...
def compute_err(n, r):
    ...
    V_qthD = VectorFunctionSpace(mesh, "DG", r-1)
    V_qth12 = FunctionSpace(mesh, "DG", r-1)
    V_qw = VectorFunctionSpace(mesh, "DG", r-1)
    ...
    bc_pw_lr = DirichletBC(V.sub(0), Constant(0.0), "on_boundary")
    bc_pw_t = DirichletBC(V.sub(0), Constant(0.0), "top")
    bc_pw_b = DirichletBC(V.sub(0), Constant(0.0), "bottom")

    bc_pth_lr = DirichletBC(V.sub(1), Constant((0.0, 0.0)), "on_boundary")
    bc_pth_t = DirichletBC(V.sub(1), Constant((0.0, 0.0)), "top")
    bc_pth_b = DirichletBC(V.sub(1), Constant((0.0, 0.0)), "bottom")
    ...
    om_err_H1 = np.zeros((n_t,))
    sig_err_L2 = np.zeros((n_t,))
    q_err_L2 = np.zeros((n_t,))

    v_err_L2[0] = np.sqrt(assemble((epw_n - dt_w)*(epw_n - dt_w) * dx))
    v_err_H1[0] = np.sqrt(assemble(dot(epw_n - v_exact, epw_n - v_exact) * dx
                                   + dot(grad(epw_n) - grad(v_exact), grad(epw_n) - grad(v_exact)) * dx))
    om_err_L2[0] = np.sqrt(assemble(inner(epth_n - dt_th, epth_n - dt_th) * dx))
    om_err_H1[0] = np.sqrt(assemble(dot(epth_n - om_exact, epth_n - om_exact) * dx
                                   + inner(gradSym(epth_n) - gradSym(om_exact), gradSym(epth_n) - gradSym(om_exact)) * dx))
    ...
    for i in range(1, n_t):
        ...
        v_err_H1[i] = np.sqrt(assemble(dot(epw_n1 - v_exact, epw_n1 - v_exact) * dx
                                       + dot(grad(epw_n1) - grad(v_exact), grad(epw_n1) - grad(v_exact)) * dx))
        om_err_L2[i] = np.sqrt(assemble(inner(epth_n1 - dt_th, epth_n1 - dt_th) * dx))
        om_err_H1[i] = np.sqrt(assemble(dot(epth_n1 - om_exact, epth_n1 - om_exact) * dx
                                        + inner(gradSym(epth_n1) - gradSym(om_exact),
                                              gradSym(epth_n1) - gradSym(om_exact)) * dx))
    ...
    om_err_max = max(om_err_H1)
    om_err_quad = np.sqrt(np.sum(dt * np.power(om_err_H1, 2)))
...
n_h = 5
n1_vec = np.array([2**(i+2) for i in range(4, n_h)])
n2_vec = np.array([2**(i+1) for i in range(4, n_h)])
