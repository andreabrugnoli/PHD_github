...
def sys(t, y):
    ...
    M_u[n_r:n_r + n_p, 0] = M[n_r:n_r + n_p, 0] + np.concatenate((mf_w, -mf_u))
    M_u[0, n_r:n_r + n_p] = M[0, n_r:n_r + n_p] + np.concatenate((mf_w, -mf_u))

    M_u[n_r:n_r + n_p, :n_r] = M[n_r:n_r + n_p, :n_r]
    M_u[:n_r, n_r:n_r + n_p] = M[:n_r, n_r:n_r + n_p]
