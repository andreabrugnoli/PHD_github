...
def compute_err(n, r):
    ...
    def j_operator(v_p, v_q, e_p, e_q):
        ...
        j_form = v_q.dx(0).dx(0) * e_p * dx - v_p * e_q.dx(0).dx(0) * dx
    ...
    sig_err_H1[0] = np.sqrt(assemble(inner(eq_n - sigma_ex, eq_n - sigma_ex) * dx
                                     +inner(eq_n.dx(0) - grad_sigex, eq_n.dx(0) - grad_sigex) * dx))
    sig_err_H2[0] = np.sqrt(assemble(inner(eq_n - sigma_ex, eq_n - sigma_ex) * dx
                                     + inner(eq_n.dx(0) - grad_sigex, eq_n.dx(0) - grad_sigex) * dx
                                     + inner(eq_n.dx(0).dx(0) - dxx_sigex, eq_n.dx(0).dx(0) - dxx_sigex) * dx))
    ...
    for i in range(1, n_t):
        ...
        sig_err_H1[i] = np.sqrt(assemble(inner(eq_n1 - sigma_ex, eq_n1 - sigma_ex) * dx
                                         + inner(eq_n1.dx(0) - grad_sigex, eq_n1.dx(0) - grad_sigex) * dx))
        sig_err_H2[i] = np.sqrt(assemble(inner(eq_n1 - sigma_ex, eq_n1 - sigma_ex) * dx
                                         + inner(eq_n1.dx(0) - grad_sigex, eq_n1.dx(0) - grad_sigex) * dx
                                         + inner(eq_n1.dx(0).dx(0) - dxx_sigex, eq_n1.dx(0).dx(0) - dxx_sigex) * dx))
    ...
    sig_err_last = sig_err_H2[-1]
    sig_err_max = max(sig_err_H2)
