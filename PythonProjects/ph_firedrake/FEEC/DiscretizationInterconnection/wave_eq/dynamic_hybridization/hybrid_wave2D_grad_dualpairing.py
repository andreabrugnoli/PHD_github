...
def compute_err(n_el, n_t, deg=1, t_fin=1, bd_cond="D"):
    ...
    def project_ex_Wnor(u_ex, W_nor):

        # project normal trace of u_e onto Vnor
        unor = TrialFunction(W_nor)
        wtan = TestFunction(W_nor)

        a_form = dot(wtan, unor)
        a = (a_form('+') + a_form('-')) * dS + a_form * ds

        L_form = dot(wtan, u_ex)
    ...
    P1til = FiniteElement("BDM", triangle, deg+1, variant="integral")
    P1tilf = FacetElement(P1til)
    ...
    v0, v1, v1til_nor, v0_tan = split(v_grad)

    e_grad = TrialFunction(V_grad)
    p0, u1, lam1til_nor, p0_tan = split(e_grad)
    ...
    dofsV0_tan_D = W0.dim() + W1.dim() + W1til_nor.dim() + np.array(dofs_D)
    dofsV0_tan_N = W0.dim() + W1.dim() + W1til_nor.dim() + np.array(dofs_N)

    dofsV0_tan_NoD = W0.dim() + W1.dim() + W1til_nor.dim() + \
    ...
    un_ex_p = project_ex_Wnor(u_ex, W1til_nor)

    en_grad.sub(2).assign(un_ex_p)

    pn_0, un_1, lamn_1til_nor, pn_0_tan = en_grad.split()
    ...
    a_form10 = m_form10(v1, u1, v0, p0) - 0.5 * dt * j_form10(v1, u1, v0, p0) \
               - 0.5 * dt * constr_loc(v0, p0, dot(v1til_nor, n_ver), dot(lam1til_nor, n_ver)) \
               - 0.5* dt * constr_global(dot(v1til_nor, n_ver), dot(lam1til_nor, n_ver), v0_tan, p0_tan)
    ...
    for ii in tqdm(range(n_t)):
        ...
        b_form10 = m_form10(v1, un_1, v0, pn_0) + 0.5 * dt * j_form10(v1, un_1, v0, pn_0) \
                   + 0.5 * dt * constr_loc(v0, pn_0, dot(v1til_nor, n_ver), dot(lam1til_nor, n_ver)) \
                   + 0.5 * dt * constr_global(dot(v1til_nor, n_ver), dot(lam1til_nor, n_ver), v0_tan, pn_0_tan)\
        ...
        pnmid_0, unmid_1, lamnmid_1til_nor, pnmid_0_tan = enmid_grad.split()
        ...
        if np.size(dofsV0_tan_D) == 0:
            ...
            y_nmid_D = (v0_tan('+') * dot(lamnmid_1til_nor('+'), n_ver('+')) + v0_tan('-') * \
                        dot(lamnmid_1til_nor('-'), n_ver('-'))) * dS \
                    + v0_tan * lamnmid_1til_nor * ds(domain=mesh)
        ...
        uex_nor_p = project_ex_Wnor(u_ex, W1til_nor)

        err_nor = h_cell * (uex_nor_p - lamn_1til_nor) ** 2
        errL2_lambdanor_vec[ii] = sqrt(assemble((err_nor('+') + err_nor('-')) * dS + err_nor * ds))

    fig = plt.figure()
    axes = fig.add_subplot(111)
    contours = quiver(lamn_1til_nor, axes=axes, cmap="inferno")
    ...
    err_lam_nor = Function(W1til_nor)
    err_lam_nor.assign(lamn_1til_nor-u_ex)
