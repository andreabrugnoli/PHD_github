...
# Post processing
u1_pp_err_deg3 = np.zeros((n_test_deg3, 2))
p0_pp_err_deg3 = np.zeros((n_test_deg3, 2))
u2_pp_err_deg3 = np.zeros((n_test_deg3, 2))
p3_pp_err_deg3 = np.zeros((n_test_deg3, ))

order_p0_pp_deg3 = np.zeros((n_test_deg3 - 1, 2))
order_u1_pp_deg3 = np.zeros((n_test_deg3 - 1, 2))
order_p3_pp_deg3 = np.zeros((n_test_deg3 - 1, ))
order_u2_pp_deg3 = np.zeros((n_test_deg3 - 1, 2))
...
p30_pp_err_deg3 = np.zeros((n_test_deg3,))
u12_pp_err_deg3 = np.zeros((n_test_deg3,))

order_p30_pp_deg3 = np.zeros((n_test_deg3 - 1,))
order_u12_pp_deg3 = np.zeros((n_test_deg3 - 1,))


for i in range(n_test_deg3):
    res_deg3 = compute_err(n_vec_deg3[i], 100, deg=DEG, bd_cond=bc_input, dim=geo_case)
    # 01 system
    ...
    # Post-processing
    u1_pp_err_deg3[i, :] = res_deg3["err_u1_pp"]
    p0_pp_err_deg3[i, :] = res_deg3["err_p0_pp"]
    u2_pp_err_deg3[i, :] = res_deg3["err_u2_pp"]
    p3_pp_err_deg3[i] = res_deg3["err_p3_pp"]
    ...
    p30_pp_err_deg3[i] = res_deg3["err_p30_pp"]
    u12_pp_err_deg3[i] = res_deg3["err_u12_pp"]
    ...
    if i>0:
        ...
        order_H01_deg3[i - 1] = np.log(H01_err_deg3[i] / H01_err_deg3[i - 1]) / np.log(h_vec_deg3[i] / h_vec_deg3[i - 1])
        ...
        order_H32_deg3[i - 1] = np.log(H32_err_deg3[i] / H32_err_deg3[i - 1]) / np.log(h_vec_deg3[i] / h_vec_deg3[i - 1])

        # Post-processing
        order_u1_pp_deg3[i - 1, 0] = np.log(u1_pp_err_deg3[i, 0] / u1_pp_err_deg3[i - 1, 0]) / np.log(
            h_vec_deg3[i] / h_vec_deg3[i - 1])
        order_u1_pp_deg3[i - 1, 1] = np.log(u1_pp_err_deg3[i, 1] / u1_pp_err_deg3[i - 1, 1]) / np.log(
            h_vec_deg3[i] / h_vec_deg3[i - 1])

        order_p0_pp_deg3[i - 1, 0] = np.log(p0_pp_err_deg3[i, 0] / p0_pp_err_deg3[i - 1, 0]) / np.log(
            h_vec_deg3[i] / h_vec_deg3[i - 1])
        order_p0_pp_deg3[i - 1, 1] = np.log(p0_pp_err_deg3[i, 1] / p0_pp_err_deg3[i - 1, 1]) / np.log(
            h_vec_deg3[i] / h_vec_deg3[i - 1])

        order_u2_pp_deg3[i - 1, 0] = np.log(u2_pp_err_deg3[i, 0] / u2_pp_err_deg3[i - 1, 0]) / np.log(
            h_vec_deg3[i] / h_vec_deg3[i - 1])
        order_u2_pp_deg3[i - 1, 1] = np.log(u2_pp_err_deg3[i, 1] / u2_pp_err_deg3[i - 1, 1]) / np.log(
            h_vec_deg3[i] / h_vec_deg3[i - 1])

        order_p3_pp_deg3[i - 1] = np.log(p3_pp_err_deg3[i] / p3_pp_err_deg3[i - 1]) / np.log(
            h_vec_deg3[i] / h_vec_deg3[i - 1])
        ...
        order_p30_pp_deg3[i - 1] = np.log(p30_pp_err_deg3[i] / p30_pp_err_deg3[i - 1]) / np.log(
            h_vec_deg3[i] / h_vec_deg3[i - 1])
        order_u12_pp_deg3[i - 1] = np.log(u12_pp_err_deg3[i] / u12_pp_err_deg3[i - 1]) / np.log(
            h_vec_deg3[i] / h_vec_deg3[i - 1])
...
# Post-processing
print("Estimated L2, Hcurl order of convergence for u_1_pp: " + str(order_u1_pp_deg3))
print("Estimated L2, H1 order of convergence for p_0_pp: " + str(order_p0_pp_deg3))

print("Estimated L2, Hdiv order of convergence for u_2_pp: " + str(order_u2_pp_deg3))
print("Estimated L2 order of convergence for p_3_pp: " + str(order_p3_pp_deg3))
...
print("Estimated L2 order of convergence for p_0_pp - p_3_pp: " + str(order_p30_pp_deg3))
print("Estimated L2 order of convergence for u_2_pp - u_1_pp: " + str(order_u12_pp_deg3))
...
if save_res:
    ...
    # Post-processing
    np.save(path_res + "u1_pp_err_deg3_" + bc_input + "_" + geo_case, u1_pp_err_deg3)
    np.save(path_res + "p0_pp_err_deg3_" + bc_input + "_" + geo_case, p0_pp_err_deg3)
    np.save(path_res + "u2_pp_err_deg3_" + bc_input + "_" + geo_case, u2_pp_err_deg3)
    np.save(path_res + "p3_pp_err_deg3_" + bc_input + "_" + geo_case, p3_pp_err_deg3)

    np.save(path_res + "order_u1_pp_deg3_" + bc_input + "_" + geo_case, order_u1_pp_deg3)
    np.save(path_res + "order_p0_pp_deg3_" + bc_input + "_" + geo_case, order_p0_pp_deg3)
    np.save(path_res + "order_u2_pp_deg3_" + bc_input + "_" + geo_case, order_u2_pp_deg3)
    np.save(path_res + "order_p3_pp_deg3_" + bc_input + "_" + geo_case, order_p3_pp_deg3)
    ...
    np.save(path_res + "p30_pp_err_deg3_" + bc_input + "_" + geo_case, p30_pp_err_deg3)
    np.save(path_res + "u12_pp_err_deg3_" + bc_input + "_" + geo_case, u12_pp_err_deg3)

    np.save(path_res + "order_p30_pp_deg3_" + bc_input + "_" + geo_case, order_p30_pp_deg3)
    np.save(path_res + "order_u12_pp_deg3_" + bc_input + "_" + geo_case, order_u12_pp_deg3)
