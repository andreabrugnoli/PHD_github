...
def compute_err(n_el, n_t, deg=1, t_fin=1, bd_cond="D", dim="2D"):
    ...
    P_1_b = BrokenElement(FiniteElement("N1curl", tetrahedron, deg))
    ...
    for ii in range(len(bc_D)):
        nodes_D = bc_D[ii].nodes

        dofs_essD = dofs_essD + list(nodes_D)

    dofs_essD = list(set(dofs_essD))
    dofs_natD = list(set(V_10.sub(1).boundary_nodes("on_boundary")).difference(set(dofs_essD)))

    dofsV0_D = V_10.sub(0).dim() + np.array(dofs_essD)
    dofsV0_N = V_10.sub(0).dim() + np.array(dofs_natD)

    dofs_essN = []

    for ii in range(len(bc_D)):
        nodes_N = bc_N[ii].nodes

        dofs_essN = dofs_essN + list(nodes_N)

    dofs_essN = list(set(dofs_essN))
    dofs_natN = list(set(V_32.sub(1).boundary_nodes("on_boundary")).difference(set(dofs_essN)))

    dofsV2_N = V_32.sub(0).dim() + np.array(dofs_essN)
    dofsV2_D = V_32.sub(0).dim() + np.array(dofs_natN)
    ...
    en_10.sub(0).assign(u0_1)
    en_10.sub(1).assign(p0_0)
    ...
    for ii in tqdm(range(n_t)):
        ...
        yhat_10 = assemble(y_nmid_ess10).vector().get_local()[dofsV0_D]
        u_midn_10 = enmid_10.vector().get_local()[dofsV0_D]

        uhat_10 = assemble(u_nmid_nat10).vector().get_local()[dofsV0_N]
        y_midn_10 = enmid_10.vector().get_local()[dofsV0_N]

        bdflow10_nat = np.dot(uhat_10, y_midn_10)
        bdflow10_ess = np.dot(yhat_10, u_midn_10)
        bdflow10_mid_vec[ii] = bdflow10_nat + bdflow10_ess

        yhat_32 = assemble(y_nmid_ess32).vector().get_local()[dofsV2_N]
        u_midn_32 = enmid_32.vector().get_local()[dofsV2_N]

        uhat_32 = assemble(u_nmid_nat32).vector().get_local()[dofsV2_D]
        y_midn_32 = enmid_32.vector().get_local()[dofsV2_D]
    ...
    dict_res = {"t_span": t_vec, "energy_ex": H_ex_vec, "energy_df": H_df_vec, "energy_3210": H_3210_vec,\
                ...
                "p0_f": pn_0, "u1_f": un_1, "p3_f": pn_3, "u2_f": un_2}
...
#
#
# dictres_file = open("results_waveL2.pkl", "wb")
