...
def feech_solution(f, mesh, r):
    ...
    V_element = FiniteElement('N1curl', mesh.ufl_cell(), r + 1)
    ...
    a_interior = (-sigma*tau + inner(u, grad(tau))
                  + inner(grad(sigma), v) + inner(curl(u), curl(v)))
    a_boundary = ((sigmatan - sigma)*vnor
                  + inner(cross(utan - u, n), cross(etanor, n))
                  + unor*(tautan - tau)
                  + inner(cross(rhonor, n), cross(vtan - v, n)))
...
def pp_solution(f, sigmatan, utan, r):
    ...
    R_element = FiniteElement('N1curl', mesh.ufl_cell(), r_pp + 1)
    ...
    a = (-inner(rho, eta) + inner(u, curl(eta))
         + inner(curl(rho), v) + div(u)*div(v))*dx
    L_interior = inner(f, v)
    L_boundary = inner(cross(utan, n), eta) - sigmatan*inner(v, n)
...
def rhonor_error(rho_e, rhonor):
    ...
    a_form = inner(cross(rhonor_p, n), cross(etanor, n))
    a = (a_form('+') + a_form('-'))*dS + a_form*ds

    L_form = -inner(rho_e, cross(etanor, n))
    L = (L_form('+') + L_form('-'))*dS + L_form*ds
    ...
    err = h*cross(rhonor_p - rhonor, n)**2
    return sqrt(assemble((err('+') + err('-'))*dS + err*ds))
...
def feech_error(N, r):
    ...
    # exact solution
    u_e_divfree = as_vector([sin(pi*x)*cos(pi*y), -cos(pi*x)*sin(pi*y), 0])
    u_e_curlfree = as_vector([sin(pi*x), sin(pi*y), sin(pi*z)])
...
def make_convergence_csv(log2N_min, log2N_max, r):
    ...
    df.to_csv('n=3_k=1_r=' + str(r) + '.csv', na_rep='---')
