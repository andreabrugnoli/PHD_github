...
plt.figure()
plt.plot(t_vec, divE2, 'r-.', label=r"\mathrm{d}^2(E^2)")
plt.xlabel(r'Time $[\mathrm{s}]$')
plt.title(r'Error divergence $E^2$')

if save_plots:
    plt.savefig(path_fig + "div_E2" + geo_case + bc_case + ".pdf", format="pdf")

plt.figure()
plt.plot(t_vec, divH2, 'b-.', label=r"\mathrm{d}^2(H^2)")
plt.xlabel(r'Time $[\mathrm{s}]$')
plt.title(r'Error divergence $H^2$')
...
plt.xlabel(r'Time $[\mathrm{s}]$')
plt.ylabel(r'$\dot{H}^{E^2 H^1} - <e^\partial_{h}, f^\partial_{h}>_{\Gamma_q} - \mathbf{u}^p \widehat{\mathbf{y}}^q$')
plt.title(r'Conservation law $\dot{H}^{E^2 H^1}$')
...
plt.xlabel(r'Time $[\mathrm{s}]$')
plt.ylabel(r'$\dot{H}^{H^2 E^1} - <e^\partial_{h}, f^\partial_{h}>_{\Gamma_p} - \mathbf{u}^q \widehat{\mathbf{y}}^p$')
...
plt.figure()
plt.plot(t_vec[1:]-dt/2, np.abs(np.diff(H_E2H1)/dt - bdflow_mid), '-v', label=r"$\dot{H}^{E^2 H^1}$")
plt.plot(t_vec[1:]-dt/2, np.abs(np.diff(H_H2E1)/dt - bdflow_mid), '--', label=r"$\dot{H}^{H^2 E^1}$")
# plt.plot(t_vec[1:]-dt/2, np.abs(np.diff(H_E1H1)/dt - bdflow_mid), '-.+', label=r"$\dot{H}^{E^1 H^1}$")
# plt.plot(t_vec[1:]-dt/2, np.abs(np.diff(H_E2H2)/dt - bdflow_mid), '--*', label=r"$\dot{H}^{E^2 H^2}$")
plt.plot(t_vec[1:]-dt/2, np.abs(np.diff(H_dual)/dt - bdflow_mid), '-.', label=r'$\dot{H}^{dual}$')
plt.xlabel(r'Time $[\mathrm{s}]$')
...
plt.figure()
plt.plot(t_vec, np.abs((H_E2H1 - H_E2H1[0]) - (H_ex-H_ex[0])), '-v', label=r'$\Delta H^{E^2 H^1}$')
plt.plot(t_vec, np.abs((H_H2E1 - H_H2E1[0]) - (H_ex-H_ex[0])), '--', label=r'$\Delta H^{H^2 E^1}$')
# plt.plot(t_vec, np.abs((H_E1H1 - H_E1H1[0]) - (H_ex-H_ex[0])), '--+', label=r'$\Delta H^{E^1 H^1}$')
# plt.plot(t_vec, np.abs((H_E2H2 - H_E2H2[0]) - (H_ex-H_ex[0])), '--*', label=r'$\Delta H^{E^2 H^2}$')
plt.plot(t_vec, np.abs((H_dual - H_dual[0]) - (H_ex-H_ex[0])), '-.', label=r'$\Delta H^{dual}$')
...
plt.figure()
plt.plot(t_vec, np.abs(H_E2H1 - H_ex), '-v', label=r'$H^{E^2 H^1}$')
plt.plot(t_vec, np.abs(H_H2E1 - H_ex), '--', label=r'$H^{H^2 E^1}$')
# plt.plot(t_vec, np.abs(H_E1H1 - H_ex), '--+', label=r'$H^{E^1 H^1}$')
# plt.plot(t_vec, np.abs(H_E2H2 - H_ex), '--*', label=r'$H^{E^2 H^2}$')
plt.plot(t_vec, np.abs(H_dual - H_ex), '-.', label=r'$H^{dual}$')
