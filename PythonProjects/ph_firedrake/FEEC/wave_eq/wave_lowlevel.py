...
from irksome import GaussLegendre, Dt, getForm, AdaptiveTimeStepper, TimeStepper, LobattoIIIA
...
def compute_sol(n_el, n_t, deg=1, t_fin=1):
    ...
    allvar = project(as_vector([v0, 0, 0, v0, 0, 0]), V)

    p0, q0, p0_d, q0_d = split(allvar)
    ...
    f_form_new, k, bcnew, bcdata = getForm(f_form, butcher_tableau, t, dt, allvar, bcs=bc)

    prob = NonlinearVariationalProblem(f_form_new, k, bcs=bcnew)
    ...
    for ii in tqdm(range(n_t)):
        ...
        for s in range(butcher_tableau.num_stages):
            for i in range(num_fields):
                allvar.dat.data[i][:] += float(dt) * b[s] * k.dat.data[num_fields * s + i][:]
