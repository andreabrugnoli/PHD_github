...
def compute_sol(n_el, n_t, t_fin):
    ...
    Vp_d = FunctionSpace(mesh, 'CG', 1)
    Vq_d = FunctionSpace(mesh, 'RT', 1)
    ...
    Es = 0.5 * (p0 * p0_d * dx + cross_2D(q0, q0_d) * dx)
...
t_vec, Ep_vec, Ed_vec, Es_vec = compute_sol(10, 100, 1)
