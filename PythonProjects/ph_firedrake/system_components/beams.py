...
class FloatCentBeam(SysPhdaeRig):
    ...
    def __init__(self, n_el, L, rho, A, E, I, m_joint=0.0, J_joint=0.0, bc='CF', offset=0):
        self.bc = bc
        ...
        vq_cen = TestFunction(Vq_cen)
        alq_cen = TrialFunction(Vq_cen)

        P_cen = 0.5*rho*A*(L**2 - x[0]**2) + rho*A*offset*(L-x[0])
        ...
        mcen_form = vq_cen * alq_cen * dx
        qcen_form = vq_cen * P_cen * alq_cen * dx

        petsc_m = assemble(m_form, mat_type='aij').M.handle
        M_FEM = np.array(petsc_m.convert("dense").getDenseArray())

        petsc_mcen = assemble(mcen_form, mat_type='aij').M.handle
        M_cen = np.array(petsc_mcen.convert("dense").getDenseArray())

        petsc_qcen = assemble(qcen_form, mat_type='aij').M.handle
        Q_cen = np.array(petsc_qcen.convert("dense").getDenseArray())
        ...
        M_fr[:, 0] = assemble(vp_x * rho * A * dx).vector().get_local()
        M_fr[:, 1] = assemble(vp_y * rho * A * dx).vector().get_local()
        M_fr[:, 2] = assemble(vp_y * rho * A * (offset + x[0]) * dx).vector().get_local()

        M_fr = M_fr[dofs2keep, :]

        M_r = np.zeros((n_rig, n_rig))
        m_beam = rho * L * A
        J_beam = assemble(rho * A * (offset + x[0])**2 * dx)
        ...
        M = la.block_diag(M_r, M_f, np.eye(nq_cen))
        M[n_rig:-nq_cen, :n_rig] = M_fr
        M[:n_rig, n_rig:-nq_cen] = M_fr.T
        ...
        jcen_form = -vp_y.dx(0) * alq_cen * dx
        petcs_jcen = assemble(jcen_form, mat_type='aij').M.handle
        Jp_cen = np.array(petcs_jcen.convert("dense").getDenseArray())[dofs2keep, :]
        ...
        J = np.zeros((n_tot, n_tot))
        J[n_rig:-nq_cen, n_rig:-nq_cen] = J_f
        J[n_rig:-nq_cen, -nq_cen:] = Jp_cen @ la.inv(M_cen)
        J[-nq_cen:, n_rig:-nq_cen] = - la.inv(M_cen) @ Jp_cen.T

        Q = np.eye(n_tot)
        Q[-nq_cen:, -nq_cen:] = Q_cen

        tau_CP = np.array([[1, 0, 0], [0, 1, L+offset], [0, 0, 1]])
        ...
        B[:n_rig, :n_rig] = np.eye(n_rig)
        B[:n_rig, n_rig:] = tau_CP.T
        B[n_rig:-nq_cen, n_rig:] = np.hstack((B_Fx, B_Fy, B_Mz))

        SysPhdaeRig.__init__(self, n_tot, 0, n_rig, n_p, n_q, E=M, J=J, B=B, Q=Q)


class FloatCentBeamNoTr(SysPhdaeRig):
    ...
    def __init__(self, n_el, L, rho, A, E, I, m_joint=0.0, J_joint=0.0, bc='CF', offset=0):
        ...
        mp_form = vp_y * alp_y * dx
        mq_form = vq_y * alq_y * dx
        m_form = mp_form + mq_form
        ...
        J = np.zeros((n_tot, n_tot))
        J[n_rig:-nq_cen, n_rig:-nq_cen] = J_f
        J[n_rig:-nq_cen, -nq_cen:] = Jp_cen
        J[-nq_cen:, n_rig:-nq_cen] = - Jp_cen.T

        Q = np.eye(n_tot)
        Q[-nq_cen:, -nq_cen:] = la.inv(M_cen) @ Q_cen
        ...
        B[:n_rig, :n_rig] = np.eye(n_rig)
        B[:n_rig, n_rig:] = tau_CP.T
        B[n_rig:-nq_cen, n_rig+1:] = np.hstack((B_Fy, B_Mz))
...
class SpatialBeam(SysPhdaeRig):
    ...
    def __init__(self, n_el, L, rho, A, E, I, Jxx, m_joint=0.0, J_joint=np.zeros((3, 3)), bc='CF'):
        ...
        if bc=='CF':
            ...
            dofs2dump = list([0, np_x, np_x + np_y - 2, np_x + np_y, n_p - 2])
...
class SpatialBeamNoTraction(SysPhdaeRig):
    ...
    def __init__(self, n_el, L, rho, A, E, I, Jxx, m_joint=0.0, J_joint=np.zeros((3, 3)), bc='CF'):
        ...
        if bc=='CF':
            ...
            dofs2dump = list([0, np_y-2, np_y, n_p - 2])
        dofs2keep = list(set(range(n_V)).difference(set(dofs2dump)))
        # print(dofs2keep)
...
def matrices_j2d(n_el, L, rho, A, bc='CF'):
...
def matrices_j3d(n_el, L, rho, A, bc='CF'):
...
def matrices_j3d_notraction(n_el, L, rho, A, bc='CF'):
...
def massmatrices_j3d(n_el, L, rho, A, bc='CF'):
