...
B_aug = np.zeros((n_V + n_lmb, 4))
B_aug[:n_Vp, 0] = B_Ffl
B_aug[:n_Vp, 1] = B_Mfl
B_aug[:n_rig, :n_rig] = tau_CP.T
B_aug[-n_rig:, 2:] = np.eye(2)
...
J_aug = np.vstack([ np.hstack([J_all, G]),
                    np.hstack([-G.T, Z_lmb])
                ])


Z_al_lmb = np.zeros((n_V, n_lmb))
Z_u_lmb = np.zeros((n_lmb, n_V))

M_aug = np.vstack([ np.hstack([M_all, Z_al_lmb]),
...
savemat(pathout + M_file, mdict={M_file: M_aug})
savemat(pathout + J_file, mdict={J_file: J_aug})
savemat(pathout + B_file, mdict={B_file: B_aug})
