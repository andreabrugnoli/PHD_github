...
B_l = B_l.vector().get_local()
B_lT = np.transpose(B_l)
B_r = B_r.vector().get_local()
...
M_p = np.array(assemble(m_p).M.handle.convert("dense").getDenseArray())
M_q = np.array(assemble(m_q).M.handle.convert("dense").getDenseArray())

D_p = np.array(assemble(j_p).M.handle.convert("dense").getDenseArray())
D_q = np.array(assemble(j_q).M.handle.convert("dense").getDenseArray())
...
e0_p = assemble(e_p_).vector().get_local()
e0_q = assemble(e_q_).vector().get_local()
...
def e_q_fenics(p, q):
    al_p_.vector()[:] = p
    al_q_.vector()[:] = q
    return assemble(e_q_).vector().get_local()


def e_p_fenics(p, q):
    al_p_.vector()[:] = p
    al_q_.vector()[:] = q
    return assemble(e_p_).vector().get_local()
...
ep_0 = assemble(e_p_).vector().get_local()
eq_0 = assemble(e_q_).vector().get_local()
...
plt.figure()
plt.plot(t_ev, H_vec, 'r-', label='non linear')
...
r_ev = mesh.coordinates.dat.data
...
#     plt.savefig(path_out + "Snap1d_n" + str(index + 1) + "Small.eps", format="eps")
#
