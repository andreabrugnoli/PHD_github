...
class Model_Reduction(object):
    ...
    def __call__(self):
        if self.verbose: print('Model reduction class of Scrimp created by Xavier Vasseur')
    ...
    def Specify_POD_Method(self,method):
        ...
        """
        assert method in ['Cholesky_QR', 'Randomized_SVD', 'R_SVD', 'SVD'], "Unknown choice for the algorithm !"
    ...
    def Specify_Weight_Matrix(self,M):
        ...
        """
        assert M.shape[0]== M.shape[1], "The weight matrix must be square"
        assert M.shape[0]== self.m,     "Wrong dimension for the weight matrix"
    ...
    def Factorize_QR_Projected_Matrix(self): 
        ...
        if self.debug: print("Factorize_QR_Projected_Matrix:",np.dot(self.PQ.T,self.PQ))
        if self.debug: print(np.allclose(np.dot(self.F_approx_basis.T,self.X_approx_basis),np.dot(self.PQ,self.PR)))
    ...
    def Factorize_Truncate_Cholesky_QR(self,relative_threshold):
        """
        Perform the Cholesky QR 2 algorithm (two steps of Cholesky QR) of the X and F snapshot matrices and deduce an approximation according to a relative threshold value.
        This decomposition has to be used if the matrices are sufficiently tall (m >= 1.5 n).
        See https://ieeexplore.ieee.org/document/7016731
        This corresponds to the serial version of the algorithm. 
        
        Map/Reduce techniques can be employed to implement in parallel this algorithm.
        See https://arxiv.org/pdf/1301.1071.pdf
        
        Warning: The matrix must be full column rank to apply this algorithm.
        """
        assert relative_threshold >= 0, "The relative threshold value should be positive"
        assert relative_threshold <= 1, "The relative threshold value should be less than one"
    ...
    def Factorize_Truncate_SVD(self,relative_threshold):
        ...
        if self.debug: print(Xs[:]/Xs[0],rx, self.threshold_X, Xs[rx]/Xs[0], relative_threshold)
        self.xs              = Xs
        ...
        if self.debug: print(Fs[:]/Fs[0], rf, self.threshold_F, Fs[rf]/Fs[0], relative_threshold)
        self.fs              = Fs
    ...
    def Randomized_SVD(self,rank):
        ...
        The approximate rank has to be given as a tentative value. 
        """
        ...
        if self.verbose: print(diffsnorm(self.X, XU, Xs, XVh))
        ...
        if self.verbose: print(diffsnorm(self.F, FU, Fs, FVh))
    ...
    def IRKA_Evaluate_Transfer_Function(self,interpolation_point,B,J,Q,R):
        ...
        # Compute the RHS matrix [TO BE IMPROVED, J-R in dot ?]
    ...
    def Plot_Decay_Singular_Values(self):
        """
        Plot the decay of the singular values of both X and F
        """
        assert self.factorize_and_truncate == 1, "Factorization and truncation must be performed before !"
        
        fig, (ax1, ax2) = plt.subplots(2,sharey=True)

        message = "Singular value decay - Matrix size (m,n)=("+str(self.X.shape[0])+","+str(self.X.shape[1])+")"
        x       = np.arange(min(self.X.shape[0],self.X.shape[1])) + 1
        ax1.semilogy(x, (self.xs[:]/self.xs[0]), 'o-', label='Energy')
        ax1.set(title=message, ylabel='Singular value')
        ax1.grid(True)
        ax1.legend()
    
        ax2.semilogy(x,(self.fs[:]/self.fs[0]),'ro-', label='Coenergy')
        ax2.set(xlabel='Index', ylabel='Singular value')
        ax2.grid(True)
        ax2.legend()
        plt.show()
        
        self.plot_decay_singular_value = 1
        
        return self.plot_decay_singular_value
    
    ## Method.
    #  @param self The object pointer.
    
    def Plot_Spectra(self,J,Q,R,M,Jr,Qr,Rr,Mr):
        """
        Plot the spectra of the global and reduced generalized eigenvalue problem
        """
        assert self.factorize_and_truncate == 1, "Factorization and truncation must be performed before !"
  
        D             = (J-R)@Q
        if sparse.issparse(M) and sparse.issparse(D):
            eigenvalue    = linalg.eigvals(D.todense(),M.todense())
        else:
            eigenvalue    = linalg.eigvals(D,M) 
        
        Dr            = (Jr-Rr)@Qr
        eigenvalue_r  = linalg.eigvals(Dr,Mr) 
       
        fig, (ax_eig) = plt.subplots(1,1)
        ax_eig.plot(np.real(eigenvalue) , np.imag(eigenvalue),  '+', label="Original model")
        ax_eig.plot(np.real(eigenvalue_r), np.imag(eigenvalue_r), 'o', label="Reduced model")
...
if __name__ == '__main__':
    ...
    F = np.random.uniform(low=-1.0, high=1.0, size=(n, m))
    X = X.T.dot(X)
    F = F.T.dot(F)
    M = np.eye(m)
